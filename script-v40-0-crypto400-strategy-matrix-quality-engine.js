window.__AYAZ_ACTIVE_VERSION__="v40.0-strategy-matrix-quality-engine";
const VERSION="v40.0 KRİPTO STRATEJİ MATRİSİ + KALİTE/BACKTEST MOTORU";
...
function contextTag(bt,p,ev,ageSec){
  ...
  const btAdaptive=bt.count>=5 && bt.pf>=1.00 && bt.net>=-0.25 && bt.fast<=60;
  const btOk=btClassic;
  ...
  modules.push({name:'Hacim',ok:volumeStrong,weak:volumeWeak,detail:`Hacim x${fmt(ev.candle.volRatio,2)} (${volumeStrong?'güçlü':volumeSoft?'normal':'zayıf'})`});
  ...
  modules.push({name:'ATR Stop/Hedef',ok:riskOk&&targetRoomOk,weak:riskOk||targetRoomOk,detail:`Stop ${pct(p.stopPct,2)}, TP1 ${fmt(p.tp1ReachR,2)}R, TP2 ${fmt(p.tp2ReachR,2)}R`});
  modules.push({name:'Aynı-Strateji Backtest',ok:btClassic,weak:btAdaptive,detail:`${ev.primary}, işlem ${bt.count}, win ${pct(bt.win,1)}, PF ${bt.pf>=20?'20+':fmt(bt.pf,2)}, fast ${pct(bt.fast,1)}`});
  ...
  const eliteOk=professionalOk && strategyPassCount>=7 && (btClassic||btAdaptive) && room>=2.0;
  const tradeOk=professionalOk;
  ...
  const tier=eliteOk?'A / ELİT':qualityRank>=72?'B / PROFESYONEL ADAY':qualityRank>=62?'C / ADAPTİF ADAY':'D / ZAYIF ADAY';
