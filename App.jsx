      ...
      <div className="candidateTop">
        <div>
          <div className="symbol">
            {signal.symbol} <span>{signal.tf}</span>
          </div>
...
export default function App() {
  ...
  async function scan() {
    ...
    if (!fx.rate) {
      setStatus("USD/TRY canlı kur alınamadı. TL hesaplama için kur zorunlu.");
      setRunning(false);
      return;
    }

    if (!selectedTfs.length) {
      setStatus("En az bir time frame seç.");
      setRunning(false);
      return;
    }
    ...
    try {
      ...
      setStatus("Sembol alınamadı: " + (err?.message || "Load failed") + " — Binance endpointleri engellenmiş olabilir.");
      setRunning(false);
      return;
    }
    ...
    for (const symbol of symbols) {
      for (const tf of selectedTfs) {
        if (stopRef.current) {
          setStatus(`Durduruldu. Kontrol edilen: ${localDone}/${scanTotal}`);
          setRunning(false);
          return;
        }
    ...
    <div className="page">
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .hero h1{font-size:32px;line-height:1.08;margin:0 0 8px;font-weight:950}
        .hero p{margin:0;color:var(--muted);font-weight:800}
        ...
        .tf{background:#0a1127;border:1px solid var(--line);border-radius:999px;padding:10px 13px;font-weight:950;color:#dce5ff;cursor:pointer}
        .tf.active{background:#17386f;border-color:#65a0ff}
        .btn{width:100%;border:1px solid #65a0ff;background:var(--blue);color:#fff;border-radius:20px;padding:17px 14px;font-size:21px;font-weight:950;cursor:pointer}
        .btn.secondary{background:#223052;border-color:#405582;margin-top:10px}
        .btn:disabled{opacity:.55}
        .dash{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin:8px 0 16px}
        .dash div{background:#0a1127;border:1px solid var(--line);border-radius:17px;padding:11px;text-align:center}
        .dash b{display:block;font-size:22px}
        .dash span{display:block;color:var(--muted);font-size:12px;font-weight:850;margin-top:4px}
        .status{margin-top:12px;background:#101936;border:1px solid var(--line);border-radius:17px;padding:12px 14px;font-size:15px;font-weight:900;color:#dce5ff}
        .progress{height:13px;background:#0b1228;border:1px solid var(--line);border-radius:999px;overflow:hidden;margin:14px 0}
        .bar{height:100%;background:#2f7df6}
        .section{border-top:1px solid var(--line);padding-top:12px;margin-top:18px}
        .section.long h2{color:#76ffa8}.section.short h2{color:#ff95a6}
        .candidate{border:2px solid #0f944e;border-radius:23px;background:#0b1228;padding:17px;margin:13px 0}
        .candidate.short{border-color:#7d1d31}
        .candidateTop{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}
        .symbol{font-size:27px;font-weight:950;line-height:1.08}
        .symbol span{color:var(--muted);font-size:18px}
        .reason{font-size:15px;line-height:1.42;color:#d3dcff;font-weight:850;margin-top:9px}
        .side{font-size:24px;font-weight:950;border-radius:18px;padding:11px 13px;min-width:88px;text-align:center}
        ...
        .metrics div{background:#0a1127;border:1px solid var(--line);border-radius:17px;padding:13px}
        .metrics b{color:var(--muted);font-size:13px;font-weight:850;display:block}
        .metrics strong{font-size:18px;font-weight:950;margin-top:5px;display:block;word-break:break-word}
      ...
      <div className="card">
        ...
        <div className="tfbar">
          {TF_LIST.map((tf) => (
            <button key={tf} className={`tf ${selectedTfs.includes(tf) ? "active" : ""}`} onClick={() => toggleTf(tf)} type="button">
              {tf === "1d" ? "Günlük" : tf}
            </button>
          ))}
        </div>
