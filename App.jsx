...
function analyzeSymbol(symbol, tf, candles, usdtry) {
  ...
  const lastVolume = volumes[i];
  const avgVolume = avgLast(volumes, RULE.volumePeriod);

  if (!atrNow || !rsiNow || !entry) return null;

  const volumeOk = lastVolume > avgVolume;
  ...
  return (
    <div className={`candidate ${isLong ? "long" : "short"}`}>
      ...
      <div className="metrics">
        <div>
          <b>Entry USDT</b>
          <strong>{fmt(signal.entry, 8)}</strong>
        </div>
        <div>
          <b>Entry TL</b>
          <strong>{money(signal.entryTl)}</strong>
        </div>
        <div>
          <b>Stop USDT</b>
          <strong>{fmt(signal.stop, 8)}</strong>
        </div>
        <div>
          <b>Stop TL</b>
          <strong>{money(signal.stopTl)}</strong>
        </div>
        <div>
          <b>TP1 USDT</b>
          <strong>{fmt(signal.tp1, 8)}</strong>
        </div>
        <div>
          <b>TP1 TL</b>
          <strong>{money(signal.tp1Tl)}</strong>
        </div>
        <div>
          <b>TP2 USDT</b>
          <strong>{fmt(signal.tp2, 8)}</strong>
        </div>
        <div>
          <b>TP2 TL</b>
          <strong>{money(signal.tp2Tl)}</strong>
        </div>
        <div>
          <b>TP3 USDT</b>
          <strong>{fmt(signal.tp3, 8)}</strong>
        </div>
        <div>
          <b>TP3 TL</b>
          <strong>{money(signal.tp3Tl)}</strong>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const longSignals = useMemo(
    () => signals.filter((s) => s.side === "LONG"),
    [signals]
  );

  const shortSignals = useMemo(
    () => signals.filter((s) => s.side === "SHORT"),
    [signals]
  );
  ...
  return (
    <div className="page">
      ...
      <div className="hero">
        ...
        <p>
          Binance canlı mum verisi + canlı USD/TRY. Güven skoru yok; sadece şartları
          tam geçen Long ve Short fırsatları listelenir.
        </p>
      </div>
      ...
      <div className="card">
        ...
        <div className="tfbar">
          {TF_LIST.map((tf) => (
            <button
              key={tf}
              className={`tf ${selectedTfs.includes(tf) ? "active" : ""}`}
              onClick={() => toggleTf(tf)}
              type="button"
            >
              {tf === "1d" ? "Günlük" : tf}
      ...
      <div className="dash">
        <div>
          <b>{usdtry ? money(usdtry) : "-"}</b>
          <span>USD/TRY {fxSource}</span>
        </div>
        <div>
          <b>{done}</b>
          <span>Kontrol</span>
        ...
        <div>
          <b>{longSignals.length}</b>
          <span>Long</span>
        </div>
        <div>
          <b>{shortSignals.length}</b>
          <span>Short</span>
        </div>
      </div>
      ...
      <div className="card section long">
        ...
        {longSignals.length ? (
          longSignals.map((signal, index) => (
            <SignalCard key={`${signal.symbol}-${signal.tf}-${index}`} signal={signal} />
          ))
        ) : (
          <p className="empty">Henüz Long sinyali yok.</p>
        )}
      </div>
      ...
      <div className="card section short">
        ...
        {shortSignals.length ? (
          ...
          <p className="empty">Henüz Short sinyali yok.</p>
        )}
      </div>
    </div>
  );
