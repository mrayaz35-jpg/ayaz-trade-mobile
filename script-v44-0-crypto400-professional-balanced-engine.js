window.__AYAZ_ACTIVE_VERSION__="v44.0-professional-balanced-trade-candidates";
const VERSION="v44.0 KRİPTO PROFESYONEL DENGELİ TRADE/ADAPTİF ADAY MOTORU";
...
const RULE={
  ...
  minStopPct:0.80, maxStopPct:9.00, atrStop:1.20, tp1R:1, tp2R:2, tp3R:3,
  maxHoldBars:40, fxMaxAgeSec:300,
  minBacktestTrades:10, minBacktestWin:35, minBacktestPf:1.10, minBacktestNetR:0.00,
  maxFastStopRate:70, minTp2Rate:0, minMfeMae:0.60,
  adaptiveMinTrades:5, adaptivePf:1.00, adaptiveNetR:-0.25, adaptiveFastStop:60, adaptiveTp2Rate:0, adaptiveMfeMae:0.50,
  minLiveVolRatio:0.50, atrBuffer:0.25, minTp1R:1.00, minTp2R:1.35
};
function maxStopAllowed(tf){return ({"15m":6.20,"30m":6.80,"1h":8.00,"2h":9.00,"4h":10.50}[tf]||8.00)}
function minBaseStopAllowed(tf){return ({"15m":1.20,"30m":1.50,"1h":1.80,"2h":2.20,"4h":2.50}[tf]||1.50)}
function minFastStopAllowed(tf){return ({"15m":1.20,"30m":1.50,"1h":1.80,"2h":2.20,"4h":2.50}[tf]||1.50)}
function minAtrStopAllowed(tf,ev){return structuralConfirmation(ev)?({"15m":0.75,"30m":0.90,"1h":1.05,"2h":1.20,"4h":1.35}[tf]||1.00):({"15m":1.00,"30m":1.15,"1h":1.30,"2h":1.45,"4h":1.60}[tf]||1.20)}
function structuralConfirmation(ev){return !!(ev&&(ev.structure?.bos||ev.structure?.choch||ev.gates?.srBreak||ev.gates?.retest||ev.gates?.microBreak||ev.profile?.type==='KIRILIM_RETEST'||ev.profile?.type==='KIRILIM_ONAY'||ev.profile?.type==='CHOCH_DONUS'))}
function directionalRoom(ev,dir){return dir==='LONG'?(ev?.levels?.roomLong||0):(ev?.levels?.roomShort||0)}
function targetQualityLabel(p,room){
  if(!p||!isFinite(p.tp1ReachR))return 'hedef yok';
  const spread=(p.tp3ReachR-p.tp2ReachR);
  if(room<1.50)return 'DAR HEDEF ALANI';
  if(p.tp1ReachR<1.0)return 'TP1 1R altı';
  if(p.tp2ReachR<1.35)return 'TP2 zayıf';
  if(spread<0.25)return 'TP2/TP3 çok yakın';
  if(room>=3.0&&p.tp2ReachR>=2.0)return 'güçlü hedef alanı';
  return 'uygun hedef alanı';
}
function finalDecisionLabel(ctx){
  if(ctx.tradeOk) return ctx.btNoData?'TEKNİK TRADE':'TRADE';
  if(ctx.adaptiveOk) return 'ADAPTİF ADAY';
  if(ctx.livePowerOk) return 'TEYİT BEKLER';
  return 'ELENDİ';
}
function hasMajorStructure(ev){return !!(ev&&(ev.structure?.bos||ev.structure?.choch||ev.gates?.srBreak||ev.gates?.retest||ev.gates?.microBreak||ev.gates?.lineBreak))}
function symmetricTrendContinuationQuality(ev,dir){
  if(!ev)return false;
  const room=directionalRoom(ev,dir);
  const major=hasMajorStructure(ev);
  // Yapısal teyit yoksa Trend Devam ancak gerçek lokasyon + kaliteli mum + hacim + yeterli hedef alanıyla geçer.
  if(major)return ev.gates?.locationOk && ev.gates?.triggerOk && (ev.gates?.volumeOk||ev.gates?.strongBody||ev.gates?.retest||ev.gates?.emaRetest) && room>=0.90;
  return ev.gates?.nearLevel && (ev.candle?.ok||ev.gates?.strongBody||ev.gates?.strongBreak) && ev.candle?.volRatio>=0.60 && ev.ind?.passed>=5 && room>=1.25;
}
function minStopAllowed(tf,ev){return structuralConfirmation(ev)?minBaseStopAllowed(tf):minFastStopAllowed(tf)}
function pTooTightRisk(entry,risk,tf){return entry>0 && (risk/entry*100)<minStopAllowed(tf,null)}
function minBtTrades(tf){return ({"15m":3,"30m":3,"1h":4,"2h":4,"4h":4}[tf]||3)}
function maxAgeAllowed(tf){return Math.floor((TFMS[tf]||3600000)/1000)+420}
function closedCandleAgeSec(row){return Math.max(0,Math.floor((now()-((row&&row.liveTime)||(row&&row.closeTime)||(row&&row.time)||now()))/1000))}
function isFreshClosedCandle(row,tf){const a=closedCandleAgeSec(row);return isFinite(a)&&a<=maxAgeAllowed(tf)}
function ageLabel(row,tf){const a=closedCandleAgeSec(row);return a<=maxAgeAllowed(tf)?String(a):`STALE ${a} sn`}
function apiAgeLabel(){return 'canlı API';}
...
let stats={done:0,total:0,sets:0,candles:0,source:'-',age:'-',stale:0,planGate:0,riskGate:0,btGate:0,passedGate:0};
...
function candleContext(c,i,dir){
  ...
  const momentumBull=bull&&x.bodyPct>=42&&x.close>p.high&&x.volRatio>=0.75;
  const momentumBear=bear&&x.bodyPct>=42&&x.close<p.low&&x.volRatio>=0.75;
  const insideBreakBull=bull&&x.close>Math.max(p.high,pp.high)&&x.bodyPct>=32&&x.volRatio>=0.55;
  const insideBreakBear=bear&&x.close<Math.min(p.low,pp.low)&&x.bodyPct>=32&&x.volRatio>=0.55;
  const strongBull=bull&&x.bodyPct>=46&&x.close>=p.close&&x.volRatio>=0.55;
  const strongBear=bear&&x.bodyPct>=46&&x.close<=p.close&&x.volRatio>=0.55;
...
function indicatorContext(c,i,dir){
  ...
  if(dir==='LONG'){
    checks=[x.close>x.ema21, x.ema9>=x.ema21*0.998, x.ema21>=p.ema21*0.999, x.rsi>=43&&x.rsi<=76, x.macdHist>=p.macdHist||x.macdHist>0, x.adx>=13, x.pdi>=x.mdi*0.82, x.volRatio>=0.55];
  }else{
    checks=[x.close<x.ema21, x.ema9<=x.ema21*1.002, x.ema21<=p.ema21*1.001, x.rsi<=57&&x.rsi>=24, x.macdHist<=p.macdHist||x.macdHist<0, x.adx>=13, x.mdi>=x.pdi*0.82, x.volRatio>=0.55];
  }
...
function mtfContext(sets,tf,dir,t=null){
  if(!sets)return {ok:false,label:'Üst TF yok',passed:0,total:0,opposite:0,neutral:0,primary:null,primaryState:'-'};
  const order={"15m":["30m","1h"],"30m":["1h","2h"],"1h":["2h","4h"],"2h":["4h"],"4h":["4h"]}[tf]||["1h","4h"];
  let passed=0,total=0,opposite=0,neutral=0,labels=[], states=[];
  for(const htf of order){
    ...
    states.push({tf:htf,state:s.state,good,bad,neutral:!good&&!bad});
    labels.push(`${htf}:${s.state}`);
  }
  // V36: LONG ve SHORT aynı standartla okunur. Ara TF tersliği tek başına eleme sebebi değildir.
  // Örnek: 15m LONG için 30m DOWN ama 1h UP ise bu "pullback içi LONG" olabilir.
  const primary=states[states.length-1]||null;
  const primaryGood=!!primary&&primary.good;
  const primaryNeutral=!!primary&&primary.neutral;
  const allOpposite=total>0 && opposite===total;
  const primaryBad=!!primary&&primary.bad;
  const ok=total>0 && !allOpposite && (primaryGood || primaryNeutral || passed>=1);
  const mode=primaryGood?'ANA_TF_UYUMLU':primaryNeutral?'ANA_TF_RANGE':(primaryBad&&passed>=1?'ARA_TF_UYUMLU_ANA_TERS':'KARISIK');
  return {ok,label:labels.join(' / ')||'Üst TF yok',passed,total,opposite,neutral,primary,primaryState:primary?primary.state:'-',mode};
}
function setupZoneName(dir,trend,levels,liquidity,x){
  ...
  if(dir==='LONG'){
    if(levels.nearSupport)return 'Destek / demand bölgesi';
    ...
    if(levels.nearResistance)return 'Direnç / supply bölgesi';
...
function gatePack(c,i,dir,trend,structure,levels,candle,liquidity,ind,mtf){
  ...
  const strongBreak=(srBreak||lineBreak||microBreak)&&strongBody&&x.volRatio>=0.45;
  ...
  const triggerOk=candle.ok||liquidity.ok||retest||strongBreak||(emaRetest&&strongBody)||microBreak;
  const volumeOk=(x.volRatio>=0.55&&candle.ok)||(x.volRatio>=0.35&&strongBody)||(x.volRatio>=0.62&&(srBreak||lineBreak||microBreak))||(liquidity.ok&&x.bodyPct>=24)||(x.bodyPct>=54&&directionOk)||(x.volRatio>=0.45&&directionOk&&triggerOk);
  const locationOk=nearLevel||emaZone||lineBreak||liquidity.ok||retest||microBreak;
  ...
  return {trendAligned,oppositeTrend,lineBreak,nearLevel,enoughRoom,emaZone,emaRetest,microBreak,srBreak,retest,strongBody,strongBreak,volumeOk,momentumOk,directionOk,triggerOk,locationOk,structureOk,groupPass,totalGroups:groups.length,groupsOk:groupPass>=4,mtfOk:mtf.ok};
}
function technicalEvent(c,i,dir,sets=null,tf='1h'){
  ...
  const indCore=ind.passed>=4;
  const indSoft=ind.passed>=3 && gates.momentumOk && (gates.volumeOk||gates.strongBody);
  ...
  const mtfSoft=mtf.ok || (structure.choch&&premiumTrigger) || (gates.lineBreak&&gates.strongBreak);
  ...
  if(mtf.ok && gates.trendAligned && symmetricTrendContinuationQuality({gates,candle,ind,levels,structure,profile:{type:'TREND_DEVAM'}},dir) && premiumTrigger && indCore && gates.groupPass>=4){
    add(long?'Trend Devam LONG':'Trend Devam SHORT','TREND_DEVAM',
      long?'Ana ve üst zaman yönü yukarı; fiyat destek/EMA/demand alanında alıcı tepkisi verdi. Retest zorunlu değildir; mum+hacim tetikleyicisi yeterlidir.':'Ana ve üst zaman yönü aşağı; fiyat direnç/EMA/supply alanında satıcı tepkisi verdi. Retest zorunlu değildir; mum+hacim tetikleyicisi yeterlidir.',
      'Trend yönünde tetikleyici mum kapanışı; retest varsa ek teyit',
      long?'En yakın geçerli dip / destek / demand / tetik mum low altı + 0.20 ATR':'En yakın geçerli tepe / direnç / supply / tetik mum high üstü + 0.20 ATR',
  ...
  const breakRetestOk=breakOk && (
    (gates.retest && candle.ok && x.volRatio>=0.55) ||
    gates.strongBreak ||
    (candle.ok&&x.volRatio>=0.95)
  );
  if(mtf.ok && gates.enoughRoom && breakRetestOk && candle.ok && (gates.volumeOk||gates.strongBreak) && premiumTrigger && indCore && gates.groupPass>=4){
  ...
  if(mtfSoft && structure.choch && gates.enoughRoom && (liquidity.ok||gates.nearLevel||gates.lineBreak||gates.microBreak) && premiumTrigger && (gates.volumeOk||gates.strongBody) && indCore && gates.groupPass>=4){
  ...
  if(mtf.ok && gates.trendAligned && gates.emaRetest && gates.enoughRoom && (gates.volumeOk||gates.strongBody) && premiumTrigger && (indCore||indSoft) && gates.groupPass>=4){
  ...
  if(mtfSoft && liquidity.ok && gates.enoughRoom && gates.directionOk && premiumTrigger && (gates.volumeOk||gates.strongBody) && (structure.bos||structure.choch||gates.microBreak) && (indCore||indSoft) && gates.groupPass>=4){
  ...
  const finalZone=ok?profileSetupZone(dir,selected,trend,levels,liquidity,x,gates):zone;
  return {ok,reason,strategies,profiles,primary,profile:selected,setupZone:finalZone,trend,structure,levels,candle,liquidity,ind,mtf,gates,tf,
...
function profileSetupZone(dir,profile,trend,levels,liquidity,x,gates){
  const type=profile?.type||'';
  const long=dir==='LONG';
  if(type==='KIRILIM_RETEST')return long?'Kırılan direnç / düşen çizgi retest bölgesi':'Kırılan destek / yükselen çizgi retest bölgesi';
  if(type==='KIRILIM_ONAY')return long?'Direnç/çizgi üstü güçlü kırılım-onay bölgesi':'Destek/çizgi altı güçlü kırılım-onay bölgesi';
  if(type==='CHOCH_DONUS')return long?'Destek/demand + karakter değişimi dönüş bölgesi':'Direnç/supply + karakter değişimi dönüş bölgesi';
  if(type==='EMA_PULLBACK')return long?'EMA21-EMA50 dinamik destek pullback bölgesi':'EMA21-EMA50 dinamik direnç pullback bölgesi';
  if(type==='LIKIDITE_RECLAIM')return long?'Alt likidite sweep + reclaim bölgesi':'Üst likidite sweep + ret bölgesi';
  if(type==='TREND_DEVAM'){
    if(long){
      if(levels.nearSupport)return 'Trend devam — destek/demand alıcı bölgesi';
      if(gates.emaRetest)return 'Trend devam — EMA dinamik destek reddi';
      if(gates.lineBreak)return 'Trend devam — mikro düşen çizgi kırılımı';
      return 'Trend devam — yapı içi alıcı tepkisi';
    }else{
      if(levels.nearResistance)return 'Trend devam — direnç/supply satıcı bölgesi';
      if(gates.emaRetest)return 'Trend devam — EMA dinamik direnç reddi';
      if(gates.lineBreak)return 'Trend devam — mikro yükselen çizgi kırılımı';
      return 'Trend devam — yapı içi satıcı tepkisi';
    }
  }
  return setupZoneName(dir,trend,levels,liquidity,x);
}
function strategyNeedsRetest(profile){return profile?.type==='KIRILIM_RETEST'}
function isRetestLanguageAllowed(profile){return profile?.type==='KIRILIM_RETEST'||profile?.type==='CHOCH_DONUS'}
function structuralRef(c,i,dir,ev){
  const x=c[i], tr=ev.trend, lv=ev.levels, liq=ev.liquidity, atr=x.atr||0;
  const recent=c.slice(Math.max(0,i-10),i+1);
  // V32: stop sadece "yakın bir pivot" değildir. Stop, işlemi gerçekten bozan yapısal seviyenin DIŞINDA olmalıdır.
  // LONG: stop destek/demand veya tetik mumunun en düşük seviyesinin altında kalır.
  // SHORT: stop direnç/supply veya tetik mumunun en yüksek seviyesinin üstünde kalır.
  if(dir==='LONG'){
    const refs=[x.low];
    const recentLows=recent.map(k=>k.low).filter(Number.isFinite);
    if(recentLows.length)refs.push(Math.min(...recentLows));
    if(Number.isFinite(lv.support))refs.push(lv.support);
    if(tr.lastLow&&Number.isFinite(tr.lastLow.v))refs.push(tr.lastLow.v);
    if(liq.ok&&Number.isFinite(liq.ref))refs.push(liq.ref);
    const below=refs.filter(v=>Number.isFinite(v)&&v<x.close);
    if(!below.length)return x.low;
    // En yakın seviye bazen mum içinde kalabilir; destek/demand varsa stop mutlaka onun da altında olmalı.
    const zoneFloor = Number.isFinite(lv.support) && lv.nearSupport ? Math.min(x.low,lv.support) : x.low;
    const closestBelow=below.sort((a,b)=>b-a)[0];
    return Math.min(closestBelow, zoneFloor);
  }else{
    const refs=[x.high];
    const recentHighs=recent.map(k=>k.high).filter(Number.isFinite);
    if(recentHighs.length)refs.push(Math.max(...recentHighs));
    if(Number.isFinite(lv.resistance))refs.push(lv.resistance);
    if(tr.lastHigh&&Number.isFinite(tr.lastHigh.v))refs.push(tr.lastHigh.v);
    if(liq.ok&&Number.isFinite(liq.ref))refs.push(liq.ref);
    const above=refs.filter(v=>Number.isFinite(v)&&v>x.close);
    if(!above.length)return x.high;
    // En yakın seviye direnç altında kalıyorsa geçersizdir; stop direnç/supply üstüne taşınır.
    const zoneCeil = Number.isFinite(lv.resistance) && lv.nearResistance ? Math.max(x.high,lv.resistance) : x.high;
    const closestAbove=above.sort((a,b)=>a-b)[0];
    return Math.max(closestAbove, zoneCeil);
  }
}
function invalidationGuard(c,i,dir,ev,stop){
  const x=c[i], lv=ev.levels;
  if(dir==='LONG'){
    const mustBelow = Number.isFinite(lv.support)&&lv.nearSupport ? Math.min(x.low,lv.support) : x.low;
    return stop < mustBelow;
  }else{
    const mustAbove = Number.isFinite(lv.resistance)&&lv.nearResistance ? Math.max(x.high,lv.resistance) : x.high;
    return stop > mustAbove;
  }
}
function targetRefs(c,i,dir,ev){
  const x=c[i], atr=x.atr||0, lv=ev.levels;
  const futureRange=c.slice(Math.max(0,i-160),i+1);
  // V36: hedefler önce gerçek destek/direnç/swing/likidite seviyelerinden gelir.
  // Yakın ana seviye 1R vermiyorsa hedef uydurulmaz; aday risk kapısında elenir.
  if(dir==='LONG'){
    const highs=pivotList(c,i,'H').map(p=>p.v).filter(v=>v>x.close).sort((a,b)=>a-b);
    const hard1=highs[0]||lv.resistance;
    const hard2=highs[1]||Math.max(lv.resistance||0, hard1||0);
    const hard3=highs[2]||Math.max(...futureRange.map(k=>k.high));
    const t1=hard1||x.close+atr;
    const t2=(hard2&&hard2>t1)?hard2:(hard1?hard1+atr:x.close+2*atr);
    const t3=(hard3&&hard3>t2)?hard3:Math.max(t2+atr,x.close+3*atr);
    return {t1,t2,t3,hardT1:!!hard1,hardT2:!!hard2,hardT3:!!hard3};
  }else{
    const lows=pivotList(c,i,'L').map(p=>p.v).filter(v=>v<x.close).sort((a,b)=>b-a);
    const hard1=lows[0]||lv.support;
    const hard2=lows[1]||Math.min(lv.support||Infinity, hard1||Infinity);
    const hard3=lows[2]||Math.min(...futureRange.map(k=>k.low));
    const t1=hard1||x.close-atr;
    const t2=(hard2&&hard2<t1)?hard2:(hard1?hard1-atr:x.close-2*atr);
    const t3=(hard3&&hard3<t2)?hard3:Math.min(t2-atr,x.close-3*atr);
    return {t1,t2,t3,hardT1:!!hard1,hardT2:!!hard2,hardT3:!!hard3};
  }
}
function plan(c,i,dir,ev=null){
  const entry=c[i].close, atr=c[i].atr||0;
  const tfKey=ev?.tf||'1h';
  let stopRaw, stop, risk, note='V36 bilimsel plan: Wilder ATR(14) + yapısal invalidasyon + ATR tampon; stop yapay genişletilmez';
  if(ev){
    const ref=structuralRef(c,i,dir,ev);
    const buf=(RULE.atrBuffer||0.25)*atr;
    stopRaw=dir==='LONG'?ref-buf:ref+buf;
  ...
  const minRiskPct=entry*minStopAllowed(tfKey,ev)/100;
  const minRiskAtr=atr*minAtrStopAllowed(tfKey,ev);
  const minRisk=Math.max(minRiskPct,minRiskAtr);
  const maxRisk=entry*maxStopAllowed(tfKey)/100;
  if(!isFinite(risk)||risk<=0){risk=atr*RULE.atrStop;note='ATR/R yedek plan'}
  let structuralRisk=risk, riskCapped=false, tooNarrow=false, targetBlocked=false;
  if(risk<minRisk){riskCapped=true;tooNarrow=true;note+=` | stop bilimsel eşiğin altında: ${fmt(risk/atr,2)} ATR < ${fmt(minAtrStopAllowed(tfKey,ev),2)} ATR / ${fmt(minRisk/entry*100,2)}%`}
  ...
  if(ev){
    ...
    const rawT1R=Math.abs(trgs.t1-entry)/risk;
    const rawT2R=Math.abs(trgs.t2-entry)/risk;
    if(trgs.hardT1 && rawT1R < RULE.minTp1R){riskCapped=true;targetBlocked=true;note+=` | TP1 gerçek direnç/destek ${fmt(rawT1R,2)}R: hedef alanı yetersiz`}
    if(trgs.hardT2 && rawT2R < RULE.minTp2R){riskCapped=true;targetBlocked=true;note+=` | TP2 gerçek likidite ${fmt(rawT2R,2)}R: hedef alanı yetersiz`}
    if(dir==='LONG'){
      t1=trgs.hardT1?trgs.t1:entry+risk;
      t2=(trgs.hardT2&&trgs.t2>t1)?trgs.t2:Math.max(t1+0.35*risk,entry+2*risk);
      t3=(trgs.hardT3&&trgs.t3>t2)?trgs.t3:Math.max(t2+0.50*risk,entry+3*risk);
    }else{
      t1=trgs.hardT1?trgs.t1:entry-risk;
      t2=(trgs.hardT2&&trgs.t2<t1)?trgs.t2:Math.min(t1-0.35*risk,entry-2*risk);
      t3=(trgs.hardT3&&trgs.t3<t2)?trgs.t3:Math.min(t2-0.50*risk,entry-3*risk);
    }
  }
  let tp1ReachR=Math.abs(t1-entry)/risk, tp2ReachR=Math.abs(t2-entry)/risk, tp3ReachR=Math.abs(t3-entry)/risk;
  // V43: hedefler sadece matematiksel 1R/2R/3R değil; gerçek destek/direnç alanı ve R ayrışmasıyla kontrol edilir.
  if(tp3ReachR-tp2ReachR<0.25){
    if(dir==='LONG') t3=Math.max(t3,t2+0.75*risk,entry+2.25*risk); else t3=Math.min(t3,t2-0.75*risk,entry-2.25*risk);
    tp3ReachR=Math.abs(t3-entry)/risk;
    note+=' | TP3, TP2’ye çok yakın olduğu için profesyonel ayrışma kuralıyla genişletildi';
  }
  if(tp1ReachR>4.0 && pTooTightRisk(entry,risk,tfKey)){
    tooNarrow=true; riskCapped=true; note+=' | çok dar stop yüzünden hedefler yapay yüksek R üretiyor';
  }
  const invalidationOk = ev ? invalidationGuard(c,i,dir,ev,stop) : true;
  if(!invalidationOk){riskCapped=true;note+=' | stop invalidasyon dışına çıkmadı: aday elenir'}
  const stopGuard = ev ? (dir==='LONG'?'Stop destek/demand ve tetik mumunun altındadır':'Stop direnç/supply ve tetik mumunun üstündedir') : 'ATR yedek stop';
  return {entry,stop,t1,t2,t3,risk,stopPct:risk/entry*100,tp1Pct:risk/entry*100,tp2Pct:2*risk/entry*100,tp3Pct:3*risk/entry*100,stopAtr:atr?risk/atr:0,minStopAtr:minAtrStopAllowed(tfKey,ev),planNote:note,structuralRiskPct:structuralRisk/entry*100,riskCapped,tooNarrow,targetBlocked,tp1ReachR,tp2ReachR,tp3ReachR,invalidationOk,stopGuard,entryRule:ev?.profile?.entryRule||'Tetikleyici mum kapanışı',stopRule:ev?.profile?.stopRule||'ATR stop',targetRule:ev?.profile?.targetRule||'1R/2R/3R'}
}
...
function sameFamilyStrategy(ev,strategyName){
  if(!ev||!ev.ok)return false;
  if(ev.strategies.includes(strategyName))return true;
  const t=ev.profile?.type||'';
  if(strategyName.includes('Kırılım-Retest') && t==='KIRILIM_ONAY')return true;
  if(strategyName.includes('Kırılım-Onay') && t==='KIRILIM_RETEST')return true;
  return false;
}
function backtest(c,dir,strategyName,sets=null,tf='1h'){
  ...
  for(let i=150;i<c.length-45;i++){
    ...
    if(!sameFamilyStrategy(ev,strategyName))continue;
    const p=plan(c,i,dir,ev);
    // V37: Backtest kapısı aynı planı test eder ama canlı final kadar boğulmaz.
    // Amaç BT=0 hatasını düzeltmek: yapısal stop, max stop ve temel hedef alanı korunur;
    // çok dar stop/TP2 filtresi canlı finalde ayrıca kalır.
    if(!p.invalidationOk||p.stopPct>maxStopAllowed(tf)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<0.70||p.tp2ReachR<1.00)continue;
    tr.push(simulate(c,i,dir,p));
    cool=5;
  }
  return summarize(tr);
}
function moduleState(ok, weak=false){return ok?'GEÇTİ':(weak?'ZAYIF':'GEÇMEDİ')}
function btStatus(bt){
  if(!bt||bt.count===0) return {code:'VERİ YOK', ok:false, weak:true, noData:true, bad:false};
  if(bt.count<10) return {code:'ÖRNEKLEM ZAYIF', ok:false, weak:true, noData:false, bad:false};
  if(bt.pf<1.00 || bt.net<0 || bt.win<30) return {code:'RED', ok:false, weak:false, noData:false, bad:true};
  if(bt.count>=20 && bt.win>=35 && bt.pf>=1.20 && bt.net>=0 && bt.fast<=45) return {code:'GÜÇLÜ', ok:true, weak:false, noData:false, bad:false};
  return {code:'ADAPTİF', ok:false, weak:true, noData:false, bad:false};
}
function contextTag(bt,p,ev,ageSec){
  ...
  const minStop=minStopAllowed(ev.tf||'1h',ev);
  const stopSideOk=p.invalidationOk!==false;
  const room=directionalRoom(ev,ev.dir||'LONG');
  const stopRangeOk=p.stopPct>=minStop && p.stopPct<=maxStop;
  const riskOk=stopRangeOk && stopSideOk && !p.riskCapped && p.structuralRiskPct<=maxStop;
  const targetQuality=targetQualityLabel(p,room);
  const targetRoomOk=p.tp1ReachR>=1.00 && p.tp2ReachR>=1.35 && room>=1.35;
  const structureOk=hasMajorStructure(ev);
  const candleOk=!!(ev.candle.ok||ev.gates.strongBody||ev.gates.strongBreak);
  const volumeStrong=ev.candle.volRatio>=1.20;
  const volumeSoft=ev.candle.volRatio>=0.90;
  const volumeWeak=ev.candle.volRatio>=0.50;
  const volumeCritical=ev.candle.volRatio<0.50;
  const liquidityStrong=!!ev.liquidity.ok;
  const liquidityOk=!!(ev.liquidity.ok||ev.gates.retest||ev.gates.srBreak||ev.gates.lineBreak||ev.gates.emaRetest);
  const indicatorOk=ev.ind.passed>=5;
  const trendOk=!!(ev.mtf.ok && (ev.gates.trendAligned||structureOk||ev.profile?.type==='EMA_PULLBACK'));
  const locationOk=!!(ev.gates.locationOk||ev.gates.nearLevel||ev.gates.emaRetest||ev.gates.retest);
  const triggerQuality=!!(ev.gates.triggerOk||candleOk||ev.gates.retest||ev.gates.emaRetest);
  const livePowerOk=trendOk && locationOk && triggerQuality && indicatorOk && targetRoomOk;

  const bts=btStatus(bt);
  const btClassic=bts.code==='GÜÇLÜ';
  const btAdaptive=bts.code==='ADAPTİF';
  const btSampleWeak=bts.code==='ÖRNEKLEM ZAYIF';
  const btNoData=bts.noData;
  const btBad=bts.bad;
  const btOk=btClassic||btAdaptive;

  const modules=[];
  modules.push({name:'Trend/MTF',ok:trendOk,weak:ev.mtf.ok,detail:`Ana TF ${ev.trend.state}, üst TF ${ev.mtf.label}`});
  modules.push({name:'Yapı/BOS-CHOCH',ok:structureOk,weak:!!(ev.structure.bos||ev.structure.choch||ev.gates.srBreak||ev.gates.lineBreak),detail:`BOS ${yn(ev.structure.bos)}, CHOCH ${yn(ev.structure.choch)}, SR ${yn(ev.gates.srBreak||ev.gates.lineBreak)}, Retest ${yn(ev.gates.retest)}, Mikro ${yn(ev.gates.microBreak)}`});
  modules.push({name:'Lokasyon/SR-Çizgi',ok:locationOk&&room>=1.20,weak:locationOk,detail:`${ev.setupZone}, hedef alanı ${fmt(room,2)} ATR`});
  modules.push({name:'İndikatör/Ortalamalar',ok:indicatorOk,weak:ev.ind.passed>=4,detail:`EMA/RSI/MACD/ADX-DI ${ev.ind.passed}/${ev.ind.total}`});
  modules.push({name:'Mum Formasyonu',ok:candleOk,weak:ev.candle.bodyPct>=30,detail:`${ev.candle.name}, gövde ${fmt(ev.candle.bodyPct,0)}%`});
  modules.push({name:'Hacim',ok:volumeSoft,weak:volumeWeak,detail:`Hacim x${fmt(ev.candle.volRatio,2)} (${volumeStrong?'güçlü':volumeSoft?'normal':volumeWeak?'zayıf':'kritik zayıf'})`});
  modules.push({name:'Likidite/SMC',ok:liquidityStrong,weak:liquidityOk,detail:`Sweep/Reclaim ${yn(ev.liquidity.ok)}, OB/FVG lokasyonla birlikte değerlendirilir`});
  modules.push({name:'ATR Stop/Hedef',ok:riskOk&&targetRoomOk&&(targetQuality==='uygun hedef alanı'||targetQuality==='güçlü hedef alanı'),weak:riskOk||targetRoomOk,detail:`Stop ${pct(p.stopPct,2)}, TP1 ${fmt(p.tp1ReachR,2)}R, TP2 ${fmt(p.tp2ReachR,2)}R, TP3 ${fmt(p.tp3ReachR,2)}R, ${targetQuality}`});
  modules.push({name:'Aynı-Strateji Backtest',ok:btClassic,weak:(btAdaptive||btSampleWeak||btNoData),detail:`${bts.code} | ${ev.primary}, işlem ${bt.count}, win ${pct(bt.win,1)}, PF ${bt.pf>=20?'20+':fmt(bt.pf,2)}, fast ${pct(bt.fast,1)}${btSampleWeak?' | örneklem zayıf':''}${btNoData?' | veri yok, canlı teknik ayrı değerlendirilir':''}`});

  const strategyPassCount=modules.filter(m=>m.ok).length;
  const weakCount=modules.filter(m=>!m.ok&&m.weak).length;
  const strategyLines=modules.map((m,i)=>`${i+1}) ${m.name}: ${moduleState(m.ok,m.weak)} | ${m.detail}`);
  const passList=modules.filter(m=>m.ok).map(m=>`${m.name}: ${m.detail}`);
  const failList=modules.filter(m=>!m.ok).map(m=>`${m.name}: ${m.weak?'ZAYIF':'GEÇMEDİ'} | ${m.detail}`);

  const criticalCandleFail=!candleOk && ev.candle.bodyPct<30;
  const coreOk=dataOk && riskOk && targetRoomOk && trendOk && locationOk && triggerQuality && indicatorOk;
  const strategyFit=structureOk || ev.profile?.type==='EMA_PULLBACK' || ev.profile?.type==='TREND_DEVAM' || ev.gates.retest || ev.gates.emaRetest;
  const technicalStrong=coreOk && strategyFit && strategyPassCount>=6 && !criticalCandleFail && room>=1.50 && (structureOk || ev.profile?.type==='EMA_PULLBACK' || ev.gates.retest || ev.gates.emaRetest);
  const technicalSoft=coreOk && strategyFit && strategyPassCount>=5 && !criticalCandleFail;

  // V44: Kötü backtest ile TRADE verilmez; veri yoksa güçlü teknik aday cezalandırılmaz, "Teknik Trade / BT yok" olarak ayrılır.
  const tradeOk=technicalStrong && !btBad && !volumeCritical && (btClassic || btNoData || (btAdaptive && volumeSoft));
  const adaptiveOk=!tradeOk && technicalSoft && !btBad;
  const eliteOk=tradeOk && strategyPassCount>=7 && (btClassic||btNoData) && room>=2.0 && volumeSoft;
  const qualityRank=strategyPassCount*10 + weakCount*4 + (room>=3?12:room>=2?8:room>=1.5?4:0) + (btClassic?14:btAdaptive?6:btNoData?4:btSampleWeak?2:0) + (volumeStrong?8:volumeSoft?4:volumeWeak?1:-8) + (btBad?-50:0);
  const tier=eliteOk?'A / ELİT':tradeOk?(btNoData?'B+ / TEKNİK TRADE - BT YOK':'B / TRADE'):adaptiveOk?'C / ADAPTİF ADAY':livePowerOk?'D / TEYİT BEKLER':'E / ELENDİ';
  const tierRank=eliteOk?4:tradeOk?3:adaptiveOk?2:livePowerOk?1:0;
  const state=tradeOk?(btNoData?'TEKNİK TRADE / BT YOK':'TRADE EDİLEBİLİR'):adaptiveOk?'ADAPTİF ADAY':livePowerOk?'TEYİT BEKLER':'ELENDİ';
  return {state,tradeOk,btOk,classicBtOk:btClassic,adaptiveBtOk:btAdaptive,btNoData,btBad,livePowerOk,mfeMae,riskOk,dataOk,maxStop,minStop,fastStopGuardOk:riskOk,targetRoomOk,nakedTrend:false,nakedTrendOk:true,primary:ev.primary,strategies:ev.strategies,professionalOk:technicalSoft,eliteOk,tier,tierRank,qualityRank,passList,failList,structureOk,volumeSoftOk:volumeSoft,volumeStrong,triggerQuality,strategyLines,strategyPassCount,weakCount,targetQuality,adaptiveOk,criticalVolumeFail:volumeCritical,criticalCandleFail};
}
...
function candidateRank(x){
  const e=x.event, c=x.context;
  let r=0;
  if(c.eliteOk)r+=80; else if(c.professionalOk)r+=55;
  r+=(c.strategyPassCount||0)*5;
  if(e.mtf.ok)r+=14;
  if(hasMajorStructure(e))r+=14;
  if(e.gates.retest)r+=8;
  if(e.structure.choch)r+=8;
  if(e.structure.bos)r+=6;
  if(e.gates.srBreak||e.gates.lineBreak)r+=6;
  if(e.liquidity.ok)r+=5;
  if(e.candle.ok)r+=5;
  if(e.candle.volRatio>=1.10)r+=5; else if(e.candle.volRatio<0.80)r-=4;
  r+=Math.min(14,(x.dir==='LONG'?e.levels.roomLong:e.levels.roomShort)*4);
  if(c.btBad)r-=60;
  if(c.classicBtOk)r+=16; else if(c.btNoData)r+=5; else if(c.adaptiveBtOk)r+=6;
  r+=Math.min(12,Math.max(0,x.bt.net));
  if(e.candle.volRatio<0.50)r-=20; else if(e.candle.volRatio<0.80)r-=6;
  if(!hasMajorStructure(e)&&e.profile?.type!=='EMA_PULLBACK')r-=12;
  if(x.bt.fast>50)r-=6;
  return r;
}
function betterCandidate(a,b){
  if(!b)return true;
  const ar=candidateRank(a),br=candidateRank(b); if(Math.abs(ar-br)>0.01)return ar>br;
  const ap=strategyPriority(a), bp=strategyPriority(b); if(ap!==bp)return ap>bp;
  if(a.context.tierRank!==b.context.tierRank)return a.context.tierRank>b.context.tierRank;
...
function analyze(sym,tf,raw,dir,sets){
  ...
  const ageSec=closedCandleAgeSec(x);
  ...
  stats.planGate++;
  const p=plan(c,i,dir,ev);
  if(p.riskCapped||p.stopPct<minStopAllowed(tf,ev)||p.stopPct>maxStopAllowed(tf)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<RULE.minTp1R||p.tp2ReachR<1.05)return null;
  if(ev.profile?.type==='TREND_DEVAM' && !symmetricTrendContinuationQuality(ev,dir))return null;
  stats.riskGate++;
  const bt=backtest(c,dir,ev.primary,preparedSets,tf);
  const context=contextTag(bt,p,ev,ageSec);
  if(context.btOk)stats.btGate++;
  if(context.state==='ELENDİ')return null;
...
function dataLine(){return `KRİPTO | Geçerli set ${stats.sets}/${stats.done} | Mum ${stats.candles} | Plan ${stats.planGate} | Risk ${stats.riskGate} | BT ${stats.btGate} | Trade ${stats.passedGate} | Kaynak ${stats.source} | Yaş ${stats.age} sn | ${VERSION}<br>${fxLine()}`}
function summary(){const cL=pools.crypto.LONG.length,cS=pools.crypto.SHORT.length;$('summary').innerHTML=`<div class="dash"><div><b>${cryptoSymbols.length}</b><span>kripto evreni</span></div><div><b>${stats.done}/${stats.total}</b><span>tarama</span></div><div><b>${stats.sets}</b><span>geçerli set</span></div><div><b>${stats.candles}</b><span>mum</span></div><div><b>${stats.planGate}</b><span>plan kapısı</span></div><div><b>${stats.riskGate}</b><span>risk kapısı</span></div><div><b>${stats.btGate}</b><span>BT kapısı</span></div><div><b>${stats.passedGate}</b><span>trade</span></div><div><b>${cL}</b><span>LONG aday</span></div><div><b>${cS}</b><span>SHORT aday</span></div></div><div class="note"><b>v44.0:</b> Profesyonel denge motoru aktif. ATR/RSI/ADX-DI Wilder/RMA mantığı kullanılır. Strateji aileleri ayrı çalışır: Trend Devam, Kırılım-Retest, Kırılım-Onay, CHOCH Dönüş, EMA Pullback ve Likidite. Retest yoksa retest dili kullanılmaz; stop destek/direnç doğru tarafında değilse aday elenir; TP/R yeni stopa göre tekrar hesaplanır. Skor ve izleme yok; yalnızca trade edilebilir ilk 10 LONG + ilk 10 SHORT listelenir. Profesyonel strateji modül motoru aktif: Her aday Trend/MTF, BOS-CHOCH, destek-direnç/çizgi, indikatör/ortalamalar, mum+hacim, likidite/SMC, ATR stop-hedef ve aynı-strateji backtest modüllerinden tek tek geçirilir. Kartta hangi modülden geçtiği, hangisinin zayıf kaldığı görünür. Final karar artık TRADE / ADAPTİF ADAY / TEYİT BEKLER / ELENDİ olarak ayrılır. Hacim, mum, backtest örneklemi, stop ve hedef ayrışması ayrı ayrı gösterilir; işlem 0 veya hacim kritik zayıfsa aday trade değil adaptif/teyit bekler olur.</div>`}
function topBy(dir){return pools.crypto[dir].slice(0,RULE.topN)}
function yn(v){return v?'EVET':'HAYIR'}
function card(x,i){const cls=x.dir==='SHORT'?'short':'long';const e=x.event;const title=x.dir==='LONG'?'Profesyonel LONG planı':'Profesyonel SHORT planı';const badge=finalDecisionLabel(x.context);return `<div class="candidate ${cls}" onclick="selectCandidate('${x.key}')"><div class="top"><div><div class="sym">${i+1}) ${displaySymbol(x)} / ${x.tf}</div><div class="model">KRİPTO ${x.dir} — ${title}</div></div><div class="badge ${x.dir.toLowerCase()}">${badge}<br><span style="font-size:13px">${e.primary}</span></div></div><div class="line"><b>Strateji:</b> ${e.primary}<br><b>Setup bölgesi:</b> ${e.setupZone}<br><b>Giriş kuralı:</b> ${x.entryRule}<br><b>Stop kuralı:</b> ${x.stopRule}<br><b>Hedef kuralı:</b> ${x.targetRule}<br><b>Trend:</b> Ana TF ${e.trend.state} | Üst TF ${e.mtf.label} | Uyum ${yn(e.mtf.ok)}<br><b>Yapı:</b> BOS ${yn(e.structure.bos)} | CHOCH ${yn(e.structure.choch)} | SR kırılım ${yn(e.gates.srBreak||e.gates.lineBreak)} | Retest ${yn(e.gates.retest)} | Mikro ${yn(e.gates.microBreak)}<br><b>Destek/Direnç:</b> Destek ${dual(x,e.levels.support)} | Direnç ${dual(x,e.levels.resistance)} | Alan ${x.dir==='LONG'?fmt(e.levels.roomLong,2):fmt(e.levels.roomShort,2)} ATR<br><b>Mum & Likidite:</b> ${e.candle.name} | Sweep/Reclaim ${yn(e.liquidity.ok)} | Hacim x${fmt(e.candle.volRatio,2)}<br><b>Kapı grupları:</b> Lokasyon ${yn(e.gates.locationOk)} | Tetik ${yn(e.gates.triggerOk)} | Momentum ${yn(e.gates.momentumOk)} | Hacim/Gövde ${yn(e.gates.volumeOk)} | Alan ${yn(e.gates.enoughRoom)} | Grup ${e.gates.groupPass}/${e.gates.totalGroups}<br><b>Final canlı kapı:</b> Üst TF ${yn(e.mtf.ok)} | Tetik kalitesi ${yn(e.candle.ok||e.liquidity.ok||e.gates.strongBreak||e.gates.retest||e.gates.emaRetest)} | Hacim/Gövde ${yn(e.gates.volumeOk)} | İndikatör ${e.ind.passed}/${e.ind.total} | Alan ${yn(e.gates.enoughRoom)}<br><b>Hesap motoru:</b> Kapalı mum | Wilder ATR/RSI/ADX | Yapısal stop | Stop Guard ${yn(x.invalidationOk)}<br><b>Bağlam seviyesi:</b> ${x.context.tier} | Strateji modülü ${x.context.strategyPassCount||0}/8<br><b>Strateji Modülleri:</b><br>${(x.context.strategyLines||[]).join('<br>')}<br><b>Geçti:</b> ${x.context.passList.slice(0,6).join(' | ')}<br><b>Geçmedi/Zayıf:</b> ${x.context.failList.length?x.context.failList.slice(0,5).join(' | '):'Kritik zayıf halka yok'}<br><b>Final kalite:</b> Risk ${yn(x.context.riskOk)} | V44 Denge + Trade/Adaptif Ayrımı ${yn(x.context.fastStopGuardOk)} | Hedef Alan ${yn(x.context.targetRoomOk)} | Backtest ${x.context.classicBtOk?'KLASİK GEÇTİ':x.context.adaptiveBtOk?'ADAPTİF GEÇTİ':'ZAYIF/ADAPTİF'} | Min/Max stop ${pct(x.context.minStop,2)} / ${pct(x.context.maxStop,2)}<br>Giriş ${dual(x,x.entry)} | Stop ${dual(x,x.stop)} | Stop ${pct(x.stopPct,2)} | TP1 ${dual(x,x.t1)} (${fmt(x.tp1ReachR,2)}R) | TP2 ${dual(x,x.t2)} (${fmt(x.tp2ReachR,2)}R) | TP3 ${dual(x,x.t3)} (${fmt(x.tp3ReachR,2)}R)<br><b>Aynı strateji backtest:</b> İşlem ${x.bt.count} | Win ${pct(x.bt.win,1)} | PF ${x.bt.pf>=20?'20+':fmt(x.bt.pf,2)} | Hızlı stop ${pct(x.bt.fast,1)} | Net ${fmt(x.bt.net,2)}R | TP2 ${pct(x.bt.tp2Rate,1)} | MFE/MAE ${fmt(x.bt.avgMfe,2)}R/${fmt(x.bt.avgMae,2)}R<br>Veri: ${x.ageSec} sn | Kaynak: ${x.source}</div><span class="pill ${x.dir==='LONG'?'green':'red'}">KRİPTO ${x.dir}</span><span class="pill blue">SKORSUZ</span><span class="pill amber">${e.profile?.type||e.primary}</span><span class="pill gray">ENTRY + STOP + TP PLAN</span></div>`}
function render(){summary();const all=allCandidates();all.forEach((x,i)=>x.key=x.key||`crypto_${x.sym}_${x.tf}_${x.dir}_${i}`);window.__map=new Map(all.map(x=>[x.key,x]));function sec(dir,label){const arr=topBy(dir);return `<div class="section ${dir==='LONG'?'long':'short'}"><h3>${label}</h3>${arr.length?arr.map((x,i)=>card(x,i)).join(''):'<p>Trade edilebilir aday yok. Profesyonel trade planı + aynı-strateji backtest kapısını geçen coin/timeframe bulunursa burada görünür.</p>'}</div>`}$('list').innerHTML=sec('LONG','KRİPTO Profesyonel İlk 10 LONG')+sec('SHORT','KRİPTO Profesyonel İlk 10 SHORT');const first=topBy('LONG')[0]||topBy('SHORT')[0];if(first&&!selected)selectCandidate(first.key,true);$('dataBox').innerHTML=dataLine()}
function metric(k,v){return `<div class="metric"><div class="k">${k}</div><div class="v">${v}</div></div>`}
function selectCandidate(key,silent=false){const x=window.__map&&window.__map.get(key);if(!x)return;selected=x;const e=x.event;$('decision').className='decision '+(x.dir==='LONG'?'long':'short');$('decision').textContent=`${displaySymbol(x)} KRİPTO ${x.dir} / ${x.tf} — ${e.primary}`;$('metrics').innerHTML=metric('Strateji',e.primary)+metric('Setup',e.setupZone)+metric('Giriş',dual(x,x.entry))+metric('Stop',dual(x,x.stop))+metric('Stop %',pct(x.stopPct,2))+metric('TP1',dual(x,x.t1)+' / '+fmt(x.tp1ReachR,2)+'R')+metric('TP2',dual(x,x.t2)+' / '+fmt(x.tp2ReachR,2)+'R')+metric('TP3',dual(x,x.t3)+' / '+fmt(x.tp3ReachR,2)+'R')+metric('Trend',e.trend.state)+metric('Üst TF',e.mtf.label)+metric('BOS / CHOCH',yn(e.structure.bos)+' / '+yn(e.structure.choch))+metric('Kapı',e.gates.groupPass+'/'+e.gates.totalGroups);$('tryPlan').innerHTML=`<b>${displaySymbol(x)} ${x.dir}</b><br><b>Neden giriş hazır?</b> ${e.reason}<br><b>Giriş:</b> ${x.entryRule}.<br><b>Stop:</b> ${x.stopRule}. Hesaplanan stop ${dual(x,x.stop)} (${pct(x.stopPct,2)}).<br><b>Hedef:</b> ${x.targetRule}. TP1 ${dual(x,x.t1)}, TP2 ${dual(x,x.t2)}, TP3 ${dual(x,x.t3)}.<br><b>Plan notu:</b> ${x.planNote}.<br><b>Stop güvenliği:</b> ${x.stopGuard}.`;renderChart(x);$('bt').innerHTML=`<div class="grid">${metric('Test edilen strateji',e.primary)+metric('İşlem',x.bt.count)+metric('Win',pct(x.bt.win,1))+metric('PF',x.bt.pf>=20?'20+':fmt(x.bt.pf,2))+metric('Hızlı stop',pct(x.bt.fast,1))+metric('Net R',fmt(x.bt.net,2))+metric('TP2 oranı',pct(x.bt.tp2Rate,1))+metric('MFE/MAE',fmt(x.bt.avgMfe,2)+'/'+fmt(x.bt.avgMae,2))+metric('Kapı durumu',x.context.state)}</div><div class="note"><b>Backtest kapısı:</b> Canlıda hangi strateji geçtiyse geçmişte yalnızca aynı strateji aranır. Aynı trend/üst TF, lokasyon, tetik, yapı kırılımı, mum/likidite, indikatör grupları, aynı kapalı mum standardı, Wilder ATR, aynı giriş kuralı, en yakın yapısal stop + ATR tampon ve aynı hedef mantığıyla R sonucu ölçülür. Puanlama, izleme ve farklı stratejiyle test yoktur.</div>`;if(!silent)$('planBox').scrollIntoView({behavior:'smooth',block:'start'})}
