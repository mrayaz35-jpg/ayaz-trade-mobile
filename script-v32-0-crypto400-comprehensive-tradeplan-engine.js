window.__AYAZ_ACTIVE_VERSION__="v32.0-comprehensive-trade-plan-engine";
const VERSION="v32.0 KRİPTO KAPSAMLI TRADE PLAN + STRUCTURAL STOP + SAME-BT MOTORU";
...
function plan(c,i,dir,ev=null){
  ...
  let stopRaw, stop, risk, note='Yapısal stop + ATR tampon | V32 stratejiye özel invalidasyon dışı stop';
  if(ev){
...
function backtest(c,dir,strategyName,sets=null,tf='1h'){
  ...
  for(let i=150;i<c.length-45;i++){
    ...
    if(p.riskCapped||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<0.85||p.tp2ReachR<1.25)continue;
...
function contextTag(bt,p,ev,ageSec){
  ...
  const riskOk=p.stopPct>=RULE.minStopPct && p.stopPct<=maxStop && !p.riskCapped && p.structuralRiskPct<=maxStop && stopSideOk && retestOkForProfile;
...
function analyze(sym,tf,raw,dir,sets){
  ...
  stats.planGate++;
  const p=plan(c,i,dir,ev);
  if(p.riskCapped||p.stopPct>maxStopAllowed(tf)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<0.80||p.tp2ReachR<1.15)return null;
  stats.riskGate++;
