...
window.__AYAZ_ACTIVE_VERSION__ = "v18.6-watch-mode";
const VERSION="v18.7 S9 NÖBET MODU — CANLI KUR - TRADE BULANA KADAR";
...
let SYMBOLS=[...DEFAULT_SYMBOLS],market={data:{},generatedAt:null},fx={rate:null,source:"-",ageSec:null,updatedAt:null,stale:false},scan={done:0,total:0,dirChecks:0,passedTech:0,btPassed:0,out:0,json:0,rest:0,stale:0,invalid:0,candles:0,restCandles:0,jsonCandles:0,validSets:0,latestCoinAgeSec:null,lastCoinSource:"-"},pool=[],selected=null;
const FX_MAX_AGE_SEC=300; // v18.7: USDT/TRY kuru 5 dakikadan eskiyse canlı yenilenir.
let watchActive=false,watchTimer=null,watchFound={LONG:null,SHORT:null},watchCycle=0;
const WATCH_RULE={
  intervalMs:60000,
  requireBoth:true,
  minBtScore:45,
...
function setFxBox(){
  ...
  const note=stale?' | UYARI: kur eski, TL yaklaşık':'';
  $("fxBox").innerHTML=`Kur: <b class="${cls}">1 USDT ≈ ${fmt(fx.rate,4)} TL</b> | Kaynak: ${fx.source} | Yaş: ${fx.ageSec??'-'} sn${note}`;
}
...
async function loadFx(force=false){
  // v18.7: market.json içindeki eski kur canlı kuru kilitlemesin. Kur 5 dakikadan eskiyse doğrudan canlı kaynaktan yenilenir.
  ...
  const sources=[
    ...
    {source:"BINANCE-API2 USDTTRY",url:"https://api2.binance.com/api/v3/ticker/price?symbol=USDTTRY",parse:j=>Number(j.price)},
  ...
  for(const s of sources){
    try{
      const j=await jfetch(s.url,8000);
      const r=s.parse(j);
      if(r>10&&r<250){fx={rate:r,source:s.source,ageSec:0,updatedAt:now(),stale:false};setFxBox();return true}
    }catch(e){}
  }
  if(fx.rate){fx.stale=true;setFxBox();return false}
  fx={rate:45.9,source:"ACIL YEDEK KUR",ageSec:null,updatedAt:null,stale:true};setFxBox();return false
}
...
function watchText(){
  const l=watchFound.LONG?`${watchFound.LONG.sym} / ${watchFound.LONG.tf}`:'bekleniyor';
  const s=watchFound.SHORT?`${watchFound.SHORT.sym} / ${watchFound.SHORT.tf}`:'bekleniyor';
  return `Nöbet: ${watchActive?'AÇIK':'KAPALI'} | Döngü: ${watchCycle} | LONG: ${l} | SHORT: ${s}`;
}
function renderWatchBox(extra=''){
  const box=$('watchBox');
  if(!box)return;
  const l=watchFound.LONG?`<b class="ok">${watchFound.LONG.sym} ${watchFound.LONG.tf}</b>`:'<span class="bad">bekleniyor</span>';
  const s=watchFound.SHORT?`<b class="ok">${watchFound.SHORT.sym} ${watchFound.SHORT.tf}</b>`:'<span class="bad">bekleniyor</span>';
  box.innerHTML=`<b>Nöbet modu:</b> ${watchActive?'<span class="ok">AÇIK</span>':'<span class="bad">KAPALI</span>'} | Döngü ${watchCycle}<br>Trade uygun LONG: ${l}<br>Trade uygun SHORT: ${s}${extra?'<br>'+extra:''}<br><span class="dim">Not: Web uygulaması telefonda gerçek arka planda sınırsız çalışamaz; ekran açık/PWA açıkken taramayı sürdürür. Gerçek kilit ekran bildirimi için sunucu/Telegram bot gerekir.</span>`;
}
...
function notifyTrade(x,done=false){
  const title=done?'Ayaz Trade: LONG + SHORT bulundu':'Ayaz Trade: Trade uygun aday bulundu';
  const body=`${x.dir} ${x.sym} / ${x.tf} | Giriş ${fmt(x.entry,priceDecimals(x.entry))} | Stop ${fmt(x.stop,priceDecimals(x.stop))} | TP2 ${fmt(x.t2,priceDecimals(x.t2))}`;
  try{ if(navigator.vibrate) navigator.vibrate(done?[250,120,250,120,250]:[180,90,180]); }catch(e){}
  try{
    if('Notification' in window && Notification.permission==='granted') new Notification(title,{body,tag:`ayaz-${x.dir}`,renotify:true});
    else if(done || !document.hasFocus()) alert(`${title}\n${body}`);
...
function registerWatchCandidate(x){
  const old=watchFound[x.dir];
  if(!old || (x.rankScore||0)>(old.rankScore||0)){
    watchFound[x.dir]=x;
    notifyTrade(x,false);
    renderWatchBox(`<span class="ok">${x.dir} bulundu: ${x.sym} / ${x.tf}</span>`);
  }
}
function watchDone(){return !!(watchFound.LONG && watchFound.SHORT)}
async function startWatchScan(){
  watchActive=true;watchFound={LONG:null,SHORT:null};watchCycle=0;
  await requestNotifyPermission();
  renderWatchBox('<span class="ok">Nöbet başlatıldı. Sistem trade uygun 1 LONG + 1 SHORT arayacak.</span>');
...
async function runScan(opts={watch:false}){
  ...
  setMeta(watch?`Nöbet taraması başladı... ${watchText()}`:"Veri bağlantısı kuruluyor...");
  setBar(0);
  await loadMarket();await loadFx(true);await getUniverse();
  scan.total=SYMBOLS.length*TFS.length;setDataBox();renderWatchBox();
  outer: for(const sym of SYMBOLS){
    for(const tf of TFS){
      if(watch && !watchActive)break outer;
      const raw=await getCandles(sym,tf);
      scan.done++;setBar(scan.done/scan.total*100);
      if(raw&&raw.length>=220){
        for(const dir of ["LONG","SHORT"]){
          scan.dirChecks++;
          const c=analyzeLast(sym,tf,raw,dir);
          if(c){
            pool.push(c);
            if(watch && isTradeReady(c)){
              registerWatchCandidate(c);
              if(watchDone()){
                pool=dedup(pool.concat([watchFound.LONG,watchFound.SHORT])).sort((a,b)=>b.rankScore-a.rankScore);
                renderList();setDataBox();setBar(100);
                watchActive=false;$('mainBtn').disabled=false;
                notifyTrade(c,true);
                setMeta(`NÖBET TAMAMLANDI: Trade uygun LONG ve SHORT bulundu. ${watchText()}`);
                renderWatchBox('<span class="ok">Nöbet tamamlandı; tarama durdu.</span>');
                return;
              }
            }
          }else scan.out++;
      ...
      setMeta(`${watch?'Nöbet':'Tarama'} ${scan.done}/${scan.total} | ${sym} ${tf} | Set ${scan.validSets} | Mum ${scan.candles} | Kaynak ${scan.lastCoinSource} | Yaş ${scan.latestCoinAgeSec===null?"-":scan.latestCoinAgeSec} sn | Teknik havuz ${pool.length} | ${watch?watchText():''}`);
      setDataBox();renderWatchBox();
      if(scan.done%8===0){renderList();await new Promise(r=>setTimeout(r,0))}
    }
  }
  pool=dedup(pool).sort((a,b)=>b.rankScore-a.rankScore);
  renderList();setDataBox();
  if(watch && watchActive && !watchDone()){
    setMeta(`Nöbet döngüsü bitti; eksik yön var. ${Math.round(WATCH_RULE.intervalMs/1000)} sn sonra tekrar taranacak. ${watchText()}`);
    renderWatchBox('<span class="amber">Henüz iki yön tamamlanmadı; yeni döngü bekleniyor.</span>');
    watchTimer=setTimeout(()=>{ if(watchActive) runScan({watch:true}); }, WATCH_RULE.intervalMs);
  }else{
    setMeta(`Tarama bitti: ${scan.done}/${scan.total} sembol/TF | Geçerli set ${scan.validSets} | Mum ${scan.candles} | REST ${scan.rest} | JSON ${scan.json} | Yön kontrolü ${scan.dirChecks} | Teknik havuz ${pool.length}`);
    $('mainBtn').disabled=false;
  }
}
