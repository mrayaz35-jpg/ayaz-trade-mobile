window.__AYAZ_ACTIVE_VERSION__="v29.0-balanced-closed-trade-engine";
const VERSION="v29.0 KRİPTO DENGELİ CLOSED CANDLE + WILDER + TRADE PLAN MOTORU";
...
const RULE={
  ...
  minBacktestTrades:8, minBacktestWin:40, minBacktestPf:1.15, minBacktestNetR:0.75,
  maxFastStopRate:32, minTp2Rate:15, minMfeMae:1.00,
  adaptiveMinTrades:6, adaptivePf:1.80, adaptiveNetR:2.50, adaptiveFastStop:25, adaptiveTp2Rate:20, adaptiveMfeMae:1.05,
  minLiveVolRatio:0.55
};
function maxStopAllowed(tf){return ({"15m":5.20,"30m":5.80,"1h":7.00,"2h":7.80,"4h":8.80}[tf]||7.00)}
function minBtTrades(tf){return ({"15m":7,"30m":8,"1h":8,"2h":9,"4h":9}[tf]||8)}
...
function gatePack(c,i,dir,trend,structure,levels,candle,liquidity,ind,mtf){
  ...
  const strongBreak=(srBreak||lineBreak||microBreak)&&strongBody&&x.volRatio>=0.55;
  const volumeOk=(x.volRatio>=0.75&&candle.ok)||(x.volRatio>=0.55&&strongBody)||(x.volRatio>=0.80&&(srBreak||lineBreak||microBreak))||(liquidity.ok&&x.bodyPct>=30)||(x.bodyPct>=62&&directionOk);
...
function contextTag(bt,p,ev,ageSec){
  ...
  const tradeOk=btOk && livePowerOk && riskOk && dataOk && p.tp1ReachR>=0.85 && p.tp2ReachR>=1.25;
...
function analyze(sym,tf,raw,dir,sets){
  ...
  const p=plan(c,i,dir,ev);
  if(p.riskCapped||p.stopPct>maxStopAllowed(tf)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<0.85||p.tp2ReachR<1.25)return null;
