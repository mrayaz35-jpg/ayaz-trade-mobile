window.__AYAZ_ACTIVE_VERSION__="v34.0-scientific-atr-stop-target-engine";
const VERSION="v34.0 KRİPTO SCIENTIFIC ATR STOP + TARGET MOTORU";
...
function mtfContext(sets,tf,dir,t=null){
  ...
  const ok=total>0 && opposite<total && (passed>=1 || neutral>=1 || neutral===total);
...
function technicalEvent(c,i,dir,sets=null,tf='1h'){
  ...
  if(mtf.ok && gates.trendAligned && gates.locationOk && gates.enoughRoom && (gates.volumeOk||gates.strongBody) && premiumTrigger && indCore && gates.groupPass>=4){
  ...
  if(mtf.ok && gates.enoughRoom && breakRetestOk && (gates.volumeOk||gates.strongBreak) && premiumTrigger && indCore && gates.groupPass>=4){
...
function targetRefs(c,i,dir,ev){
  ...
  // V34: hedefler önce gerçek destek/direnç/swing/likidite seviyelerinden gelir.
...
function plan(c,i,dir,ev=null){
  ...
  let stopRaw, stop, risk, note='V34 bilimsel plan: Wilder ATR(14) + yapısal invalidasyon + ATR tampon; stop yapay genişletilmez';
  if(ev){
