window.__AYAZ_ACTIVE_VERSION__="v23.0-crypto-context-noscore";
const VERSION="v23.0 KRİPTO BAĞLAM MOTORU SKORSUZ";
const TFS=["15m","30m","1h","2h","4h"];
const TFMS={"15m":900000,"30m":1800000,"1h":3600000,"2h":7200000,"4h":14400000};
const CRYPTO_LIMIT=400;
const RULE={
  limit15:900, limit1h:650, maxCryptoAgeSec:420, topN:7, watchMs:65000,
  minStopPct:0.45, maxStopPct:8.50, atrStop:1.20, tp1R:1, tp2R:2, tp3R:3,
  maxHoldBars:40, fxMaxAgeSec:300, minBacktestTrades:5, minPf:1.05
};
...
function trendSnapshot(raw){if(!raw||raw.length<160)return null;const c=enrich(raw);const i=c.length-1;return trendContext(c,i)}
function mtfContext(sets,tf,dir){
  ...
  for(const htf of order){const s=trendSnapshot(sets[htf]);if(!s)continue;total++;const good=dir==='LONG'?s.state!=='DOWN'||s.breakDownLine:s.state!=='UP'||s.breakUpLine;if(good)passed++;labels.push(`${htf}:${s.state}`)}
...
function technicalEvent(c,i,dir,sets=null,tf='1h'){
  ...
  const trend=trendContext(c,i), structure=structureContext(c,i,dir,trend), levels=levelContext(c,i,dir,trend), candle=candleContext(c,i,dir), liquidity=liquidityContext(c,i,dir,levels), ind=indicatorContext(c,i,dir), mtf=mtfContext(sets,tf,dir);
  const atr=x.atr||0;
  const long=dir==='LONG';
  ...
  const strategies=[];
  if(mtf.ok && trendAligned && indicatorCore && structureBreak && enoughRoom && (candle.ok||lineBreak)) strategies.push(long?'Trend devam LONG':'Trend devam SHORT');
  if(mtf.ok && trendAligned && indicatorLoose && nearLevel && emaSide && candleOrSweep && enoughRoom) strategies.push(long?'Destek pullback LONG':'Direnç pullback SHORT');
  if(mtf.ok && oppositeTrend && structure.choch && lineBreak && indicatorLoose && candleOrSweep) strategies.push(long?'Bullish CHOCH dönüş LONG':'Bearish CHOCH dönüş SHORT');
  ...
  if(mtf.ok && liquidity.ok && indicatorLoose && (structureBreak||emaSide) && enoughRoom) strategies.push(long?'Likidite reclaim LONG':'Likidite sweep SHORT');
  if(trend.state==='RANGE' && mtf.ok && structure.bos && indicatorCore && (candle.ok||liquidity.ok) && enoughRoom) strategies.push(long?'Range kırılım LONG':'Range kırılım SHORT');
...
function backtest(c,dir){const tr=[];let cool=0;for(let i=150;i<c.length-45;i++){if(cool>0){cool--;continue}const ev=technicalEvent(c,i,dir,null,'BT');if(!ev.ok)continue;const p=plan(c,i,dir);tr.push(simulate(c,i,dir,p));cool=6}return summarize(tr)}
function contextTag(bt,p,ev,ageSec){
  ...
  const robust=bt.count>=RULE.minBacktestTrades && bt.pf>=1.20 && bt.net>0 && bt.fast<=35 && mfeMae>=1.05 && riskOk && dataOk;
  const usable=bt.count>=3 && bt.pf>=1.00 && bt.net>=0 && riskOk && dataOk;
  const state=robust?'TAM BAĞLAM':usable?'UYGUN BAĞLAM':'ZAYIF BT';
  return {state,robust,usable,mfeMae,riskOk,dataOk,primary:ev.primary,strategies:ev.strategies};
}
...
function betterCandidate(a,b){
  if(!b)return true;
  const ar=a.context.robust?2:a.context.usable?1:0, br=b.context.robust?2:b.context.usable?1:0;
...
function analyze(sym,tf,raw,dir,sets){
  ...
  if(bt.count<2)return null;
  const context=contextTag(bt,p,ev,ageSec);
  if(!context.usable && !context.robust)return null;
...
function render(){summary();const all=allCandidates();all.forEach((x,i)=>x.key=x.key||`crypto_${x.sym}_${x.tf}_${x.dir}_${i}`);window.__map=new Map(all.map(x=>[x.key,x]));function sec(dir,label){const arr=topBy(dir);return `<div class="section ${dir==='LONG'?'long':'short'}"><h3>${label}</h3>${arr.length?arr.map((x,i)=>card(x,i)).join(''):'<p>Aday yok. Teknik kapıdan geçen veri bulunursa burada görünür.</p>'}</div>`}$('list').innerHTML=sec('LONG','KRİPTO Trade Edilebilir İlk 7 LONG')+sec('SHORT','KRİPTO Trade Edilebilir İlk 7 SHORT');const first=topBy('LONG')[0]||topBy('SHORT')[0];if(first&&!selected)selectCandidate(first.key,true);$('dataBox').innerHTML=dataLine()}
function metric(k,v){return `<div class="metric"><div class="k">${k}</div><div class="v">${v}</div></div>`}
function selectCandidate(key,silent=false){const x=window.__map&&window.__map.get(key);if(!x)return;selected=x;const e=x.event;$('decision').className='decision '+(x.dir==='LONG'?'long':'short');$('decision').textContent=`${displaySymbol(x)} KRİPTO ${x.dir} / ${x.tf} — ${e.primary}`;$('metrics').innerHTML=metric('Giriş',dual(x,x.entry))+metric('Stop',dual(x,x.stop))+metric('Stop %',pct(x.stopPct,2))+metric('TP1',dual(x,x.t1))+metric('TP2',dual(x,x.t2))+metric('TP3',dual(x,x.t3))+metric('Bağlam',x.context.state)+metric('Strateji',e.primary)+metric('Trend',e.trend.state)+metric('BOS / CHOCH',yn(e.structure.bos)+' / '+yn(e.structure.choch))+metric('Mum',e.candle.name)+metric('Sweep',yn(e.liquidity.ok));$('tryPlan').innerHTML=`<b>${displaySymbol(x)} ${x.dir}</b><br>Bağlamdan geçti: ${e.reason}.<br>Plan: giriş ${dual(x,x.entry)}, stop ${dual(x,x.stop)}, TP2 ${dual(x,x.t2)}. Stop mantığı: ${x.planNote}. Bu sürümde sayısal skor yoktur; aday, trend-yapı-lokasyon-mum-likidite-indikatör bağlarının somut kurallarını geçtiği için listelenir.`;renderChart(x);$('bt').innerHTML=`<div class="grid">${metric('İşlem',x.bt.count)+metric('Win',pct(x.bt.win,1))+metric('PF',x.bt.pf>=20?'20+':fmt(x.bt.pf,2))+metric('Hızlı stop',pct(x.bt.fast,1))+metric('Net R',fmt(x.bt.net,2))+metric('TP2 oranı',pct(x.bt.tp2Rate,1))+metric('Backtest durumu',x.context.state)+metric('Strateji',e.primary)+metric('Üst TF',e.mtf.label)}</div><div class="note">Backtest aynı bağlam kapısını geçmiş mumlarda uygular: trend → üst TF → BOS/CHOCH → destek/direnç/trend çizgisi → mum/sweep → indikatör kuralları → ATR/R plan. Burada puanlama yok; yalnızca şartların geçip geçmediği ve geçmiş sonuç sağlığı gösterilir.</div>`;if(!silent)$('planBox').scrollIntoView({behavior:'smooth',block:'start'})}
...
async function startCryptoWatchScan(){if(scanRunning||watchActive)return;watchActive=true;cycle=0;resetPools();setButtons();setMeta('KRİPTO dinamik nöbet başladı. İlk 7 LONG/SHORT her döngüde skorsuz bağlam kapısı + backtest sağlık şartlarıyla revize olur.');await runMarketScan(true)}
