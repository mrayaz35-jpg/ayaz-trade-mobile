window.__AYAZ_ACTIVE_VERSION__="v37.0-balanced-bt-data-engine";
const VERSION="v37.0 KRİPTO BALANCED BT + DATA AGE + SYMMETRIC SCIENTIFIC MOTOR";
...
const RULE={
  ...
  minBacktestTrades:3, minBacktestWin:34, minBacktestPf:1.00, minBacktestNetR:-0.25,
  maxFastStopRate:55, minTp2Rate:5, minMfeMae:0.75,
  adaptiveMinTrades:2, adaptivePf:1.18, adaptiveNetR:0.50, adaptiveFastStop:55, adaptiveTp2Rate:5, adaptiveMfeMae:0.70,
  minLiveVolRatio:0.35, atrBuffer:0.25, minTp1R:1.00, minTp2R:1.50
};
...
function symmetricTrendContinuationQuality(ev,dir){
  ...
  if(major)return ev.gates?.locationOk && ev.gates?.triggerOk && (ev.gates?.volumeOk||ev.gates?.strongBody) && room>=1.05;
  return ev.gates?.nearLevel && ev.candle?.ok && ev.candle?.volRatio>=0.75 && ev.ind?.passed>=5 && room>=1.60;
}
...
function contextTag(bt,p,ev,ageSec){
  ...
  const nakedTrendOk=!nakedTrend || (p.tp1ReachR>=1.15 && p.tp2ReachR>=1.65 && (ev.liquidity.ok||ev.gates.emaRetest||ev.gates.nearLevel));
  ...
  const livePowerOk=ev.mtf.ok && ev.gates?.groupsOk && ev.gates.groupPass>=4 && ev.ind.passed>=4 && ev.gates.enoughRoom && ev.gates.triggerOk && ev.gates.locationOk && (ev.gates.volumeOk||ev.gates.strongBody) && triggerQuality;
  ...
  const adaptiveBtOk=bt.count>=RULE.adaptiveMinTrades && bt.count<Math.max(RULE.minBacktestTrades,minBtTrades(ev.tf||'1h')) && bt.pf>=RULE.adaptivePf && bt.net>=RULE.adaptiveNetR && bt.fast<=RULE.adaptiveFastStop && bt.tp2Rate>=RULE.adaptiveTp2Rate && mfeMae>=RULE.adaptiveMfeMae && ev.gates.groupPass>=5 && riskOk && livePowerOk;
  const btOk=classicBtOk||adaptiveBtOk;
  const tradeOk=btOk && livePowerOk && riskOk && dataOk && p.tp1ReachR>=0.80 && p.tp2ReachR>=1.15;
  const state=tradeOk?'TRADE EDİLEBİLİR':'ELENDİ';
  return {state,tradeOk,btOk,classicBtOk,adaptiveBtOk,livePowerOk,mfeMae,riskOk,dataOk,maxStop,minStop,fastStopGuardOk,targetRoomOk,nakedTrend,nakedTrendOk,primary:ev.primary,strategies:ev.strategies};
}
...
function analyze(sym,tf,raw,dir,sets){
  ...
  stats.planGate++;
  const p=plan(c,i,dir,ev);
  if(p.riskCapped||p.stopPct<minStopAllowed(tf,ev)||p.stopPct>maxStopAllowed(tf)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<RULE.minTp1R||p.tp2ReachR<RULE.minTp2R)return null;
