window.__AYAZ_ACTIVE_VERSION__="v41.0-professional-decision-target-engine";
const VERSION="v41.0 KRİPTO PROFESYONEL KARAR + HEDEF MOTORU";
...
function finalDecisionLabel(ctx){
  if(ctx.tradeOk) return 'TRADE';
  if(ctx.professionalOk) return 'ADAPTİF ADAY';
...
function plan(c,i,dir,ev=null){
  ...
  // V41: hedefler sadece matematiksel 1R/2R/3R değil; gerçek destek/direnç alanı ve R ayrışmasıyla kontrol edilir.
...
function contextTag(bt,p,ev,ageSec){
  ...
  const volumeStrong=ev.candle.volRatio>=1.10;
  const volumeSoft=ev.candle.volRatio>=0.70;
  const volumeWeak=ev.candle.volRatio>=0.50;
  const liquidityOk=!!(ev.liquidity.ok||ev.gates.retest||ev.gates.srBreak||ev.gates.lineBreak);
  ...
  const btClassic=bt.count>=20 && bt.win>=35 && bt.pf>=1.20 && bt.net>=0 && bt.fast<=45 && mfeMae>=0.60;
  const btAdaptive=bt.count>=10 && bt.pf>=1.00 && bt.net>=-0.25 && bt.fast<=60;
  const btSampleWeak=bt.count>0 && bt.count<10;
  ...
  modules.push({name:'Hacim',ok:volumeSoft,weak:volumeWeak,detail:`Hacim x${fmt(ev.candle.volRatio,2)} (${volumeStrong?'güçlü':volumeSoft?'normal':'zayıf/kritik'})`});
  modules.push({name:'Likidite/SMC',ok:liquidityOk,weak:locationOk,detail:`Sweep/Reclaim ${yn(ev.liquidity.ok)}, OB/FVG lokasyonla birlikte değerlendirilir`});
  modules.push({name:'ATR Stop/Hedef',ok:riskOk&&targetRoomOk&&targetQuality==='uygun hedef alanı'||riskOk&&targetRoomOk&&targetQuality==='güçlü hedef alanı',weak:riskOk||targetRoomOk,detail:`Stop ${pct(p.stopPct,2)}, TP1 ${fmt(p.tp1ReachR,2)}R, TP2 ${fmt(p.tp2ReachR,2)}R, TP3 ${fmt(p.tp3ReachR,2)}R, ${targetQuality}`});
  modules.push({name:'Aynı-Strateji Backtest',ok:btClassic,weak:btAdaptive||btSampleWeak,detail:`${ev.primary}, işlem ${bt.count}, win ${pct(bt.win,1)}, PF ${bt.pf>=20?'20+':fmt(bt.pf,2)}, fast ${pct(bt.fast,1)}${btSampleWeak?' | örneklem zayıf':''}`});
  ...
  const criticalVolumeFail=ev.candle.volRatio<0.50;
  ...
  const professionalOk=coreOk && strategyFit && strategyPassCount>=5;
  const tradeOk=professionalOk && btOk && !criticalVolumeFail && !criticalCandleFail;
  const adaptiveOk=professionalOk && !tradeOk;
  const eliteOk=tradeOk && strategyPassCount>=7 && btClassic && room>=2.0;
  const qualityRank=strategyPassCount*10 + weakCount*4 + (room>=3?12:room>=2?8:room>=1.5?4:0) + (btClassic?12:btAdaptive?5:0) + (volumeStrong?6:volumeSoft?3:0);
  const tier=eliteOk?'A / ELİT':tradeOk?'B / TRADE':adaptiveOk?'C / ADAPTİF ADAY':livePowerOk?'D / TEYİT BEKLER':'E / ELENDİ';
  const tierRank=eliteOk?4:tradeOk?3:adaptiveOk?2:livePowerOk?1:0;
  const state=tradeOk?'TRADE EDİLEBİLİR':adaptiveOk?'ADAPTİF ADAY':livePowerOk?'TEYİT BEKLER':'ELENDİ';
  return {state,tradeOk,btOk,classicBtOk:btClassic,adaptiveBtOk:btAdaptive,livePowerOk,mfeMae,riskOk,dataOk,maxStop,minStop,fastStopGuardOk:riskOk,targetRoomOk,nakedTrend:false,nakedTrendOk:true,primary:ev.primary,strategies:ev.strategies,professionalOk,eliteOk,tier,tierRank,qualityRank,passList,failList,structureOk,volumeSoftOk:volumeSoft,volumeStrong,triggerQuality,strategyLines,strategyPassCount,weakCount,targetQuality,adaptiveOk,criticalVolumeFail,criticalCandleFail};
}
