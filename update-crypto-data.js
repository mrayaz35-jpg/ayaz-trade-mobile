...
function getJson(url){return new Promise((resolve,reject)=>{https.get(url,{headers:{"User-Agent":"ayaz-trade-v15-0"}},res=>{let data="";res.on("data",d=>data+=d);res.on("end",()=>{try{resolve(JSON.parse(data))}catch(e){reject(e)}})}).on("error",reject)})}
...
(async()=>{
  ...
  const out={generatedAt:new Date().toISOString(),source:"data-api.binance.vision",mode:"v15-professional-technical-quality",universeLimit:UNIVERSE_LIMIT,symbolCount:symbols.length,symbols,fx,data:{}};
