...
const VERSION="MACD RSI MA ATR";
const API="https://api.binance.com";
const RULE={atrStop:1.5,tp1R:2,tp2R:3,tp3R:5};
let usdtry=0, stopScan=false;
...
async function loadUsdTry(){
  try{
    const d=await jfetch("https://open.er-api.com/v6/latest/USD",12000);
    usdtry=Number(d?.rates?.TRY||0);
  }catch(e){
    try{
      const d=await jfetch("https://api.frankfurter.app/latest?from=USD&to=TRY",12000);
      usdtry=Number(d?.rates?.TRY||0);
    }catch(e2){usdtry=0}
  }
  updateDash();
...
async function symbolsFromBinance(maxCount,minVol){
  const [info,tickers]=await Promise.all([
    jfetch(API+"/api/v3/exchangeInfo",25000),
    jfetch(API+"/api/v3/ticker/24hr",25000)
  ]);
  ...
  return tickers.filter(t=>active.has(t.symbol))
    .filter(t=>Number(t.quoteVolume)>=minVol)
    .sort((a,b)=>Number(b.quoteVolume)-Number(a.quoteVolume))
    .slice(0,maxCount).map(t=>t.symbol);
}
...
function ema(values,period){
  ...
  for(let i=0;i<values.length;i++){
    if(i<period-1)out.push(null); else if(i===period-1)out.push(prev); else{prev=values[i]*k+prev*(1-k);out.push(prev)}
  }
  return out;
}
...
function atr(candles,period=14){
  ...
  for(let i=0;i<candles.length;i++){
    ...
    else{const h=candles[i].h,l=candles[i].l,pc=candles[i-1].c; tr.push(Math.max(h-l,Math.abs(h-pc),Math.abs(l-pc)))}
  }
...
function analyze(symbol,tf,c){
  ...
  return {
    symbol,tf,side,entry,atr:atrNow,rsi:rs[i],
    stop:side==="LONG"?entry-risk:entry+risk,
    tp1:side==="LONG"?entry+risk*RULE.tp1R:entry-risk*RULE.tp1R,
    tp2:side==="LONG"?entry+risk*RULE.tp2R:entry-risk*RULE.tp2R,
    tp3:side==="LONG"?entry+risk*RULE.tp3R:entry-risk*RULE.tp3R,
    reason:"EMA 9/21/50/200 + MACD + RSI + hacim şartları tam geçti"
  };
}
function card(s){
  const cls=s.side==="LONG"?"long":"short", sideCls=s.side==="LONG"?"long":"short";
  const tl=v=>usdtry?money(v*usdtry):"-";
  const div=document.createElement("div");
  div.className="candidate "+(s.side==="SHORT"?"short":"");
  div.innerHTML=`<div class="top"><div><div class="sym">${s.symbol} <span class="dim">${s.tf}</span></div><div class="line">${s.reason}</div></div><div class="side ${sideCls}">${s.side}</div></div>
  ...
  <div class="grid">
    <div class="metric"><div class="k">Entry USDT</div><div class="v">${fmt(s.entry,8)}</div></div>
    <div class="metric"><div class="k">Entry TL</div><div class="v">${tl(s.entry)}</div></div>
    <div class="metric"><div class="k">Stop USDT / TL</div><div class="v">${fmt(s.stop,8)} / ${tl(s.stop)}</div></div>
    <div class="metric"><div class="k">TP1 USDT / TL</div><div class="v">${fmt(s.tp1,8)} / ${tl(s.tp1)}</div></div>
    <div class="metric"><div class="k">TP2 USDT / TL</div><div class="v">${fmt(s.tp2,8)} / ${tl(s.tp2)}</div></div>
    <div class="metric"><div class="k">TP3 USDT / TL</div><div class="v">${fmt(s.tp3,8)} / ${tl(s.tp3)}</div></div>
  </div>`;
  return div;
}
function clearLists(){
  $("longList").innerHTML='<p class="dim">Henüz sinyal yok.</p>';
  $("shortList").innerHTML='<p class="dim">Henüz sinyal yok.</p>';
  counts={long:0,short:0,done:0,total:0}; updateDash();
}
function addSignal(s){
  const target=s.side==="LONG"?$("longList"):$("shortList");
  if(target.querySelector("p"))target.innerHTML="";
  target.appendChild(card(s));
  if(s.side==="LONG")counts.long++; else counts.short++;
  updateDash();
}
async function scan(){
  if(scanRunning)return;
  scanRunning=true; stopScan=false; clearLists();
  $("scanBtn").disabled=true;
  setStatus("Canlı USD/TRY alınıyor...");
  await loadUsdTry();
  const tfs=[...document.querySelectorAll(".tf:checked")].map(x=>x.value);
  const maxCount=Number($("limit").value||400);
  const minVol=Number($("minVolume").value||0);
  const candleLimit=Number($("candleLimit").value||300);
  setStatus("Binance canlı USDT sembolleri alınıyor...");
  let syms=[];
  try{syms=await symbolsFromBinance(maxCount,minVol)}catch(e){setStatus("Sembol alınamadı: "+e.message); scanRunning=false; $("scanBtn").disabled=false; return}
  ...
  for(const sym of syms){
    for(const tf of tfs){
      ...
      try{
        const c=await klines(sym,tf,candleLimit);
        const s=analyze(sym,tf,c);
        if(s)addSignal(s);
      }catch(e){}
      counts.done++; updateDash();
      await sleep(90);
    }
  }
  setStatus(`Tarama bitti. Long: ${counts.long}, Short: ${counts.short}.`);
  scanRunning=false; $("scanBtn").disabled=false;
}
$("scanBtn").onclick=scan;
$("stopBtn").onclick=()=>{stopScan=true};
$("clearBtn").onclick=clearLists;
