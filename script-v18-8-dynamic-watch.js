...
window.__AYAZ_ACTIVE_VERSION__ = "v18.8-dynamic-watch-top5";
const VERSION="v18.8 S9 DİNAMİK NÖBET — EN İYİ 5+5";
...
async function runScan(opts={watch:false}){
  const watch=!!opts.watch;if(watch)watchCycle++;
  ...
  if(watch){
    ...
    if(watchActive)watchTimer=setTimeout(()=>{if(watchActive)runScan({watch:true})},WATCH_RULE.intervalMs);
  }else{
