const VERSION="v16.7 teknik kapı + backtest top 5";
...
const RULE={
  ...
  limit:700,
  spotTry:10000,
  // v16.7: Önce SADECE objektif teknik kapı çalışır.
  // Teknik kapı: dataAge≤300sn, layer≥8/14, technical≥60, execution≥65, TP2 alanı≥1.80R, 0.80≤stopATR≤2.20.
  // Teknik kapıyı geçenlerin backtesti hesaplanır; final yayın backtest dayanıklılığı en yüksek 5 LONG + 5 SHORT olur.
  techMin:60,
  execMin:65,
  minLayerHits:8,
  stopAtrIdealMin:0.80,
  stopAtrIdealMax:2.20,
  stopAtrHardMin:0.60,
  stopAtrHardMax:3.20,
  minPoolTp2:1.80,
  minBtCount:30,
...
function setDataBox(){const ok=market&&Object.keys(market.data||{}).length;$("dataBox").innerHTML=`Veri: <b class="${ok?'ok':'bad'}">${ok?'BAĞLI':'BEKLEMEDE'}</b> | Coin: ${SYMBOLS.length} | Mum: ${Object.values(market.data||{}).reduce((s,v)=>s+TFS.reduce((a,tf)=>a+((v&&v[tf]&&v[tf].length)||0),0),0)} | v16.7 teknik kapı + backtest top 5`}
...
function candleScore(c,i,dir){const x=c[i],p=c[i-1]||x;const range=Math.max(x.high-x.low,1e-9),body=Math.abs(x.close-x.open),upper=x.high-Math.max(x.close,x.open),lower=Math.min(x.close,x.open)-x.low;const bodyRatio=body/range,upperRatio=upper/range,lowerRatio=lower/range,closePos=(x.close-x.low)/range;let s=0;if(bodyRatio>=.45)s+=20;if(dir==="LONG"){if(lowerRatio>=.35)s+=25;if(closePos>=.65)s+=25;if(x.close>p.high)s+=20;if(p.close<p.open&&x.close>x.open&&body>Math.abs(p.close-p.open))s+=20}else{if(upperRatio>=.35)s+=25;if(closePos<=.35)s+=25;if(x.close<p.low)s+=20;if(p.close>p.open&&x.close<x.open&&body>Math.abs(p.close-p.open))s+=20}return clamp(s,0,100)}
...
function smcScore(c,i,dir){const x=c[i],a=x.atr||1,prior=c.slice(Math.max(0,i-40),i);const minL=Math.min(...prior.map(z=>z.low)),maxH=Math.max(...prior.map(z=>z.high));let fvg=0,ob=0,sweep=0;if(i>3){const a2=c[i-2];if(dir==="LONG"&&a2.high<x.low){const size=(x.low-a2.high)/a;if(size>=.15)fvg=30}if(dir==="SHORT"&&a2.low>x.high){const size=(a2.low-x.high)/a;if(size>=.15)fvg=30}}for(let k=Math.max(1,i-16);k<i;k++){const z=c[k],n=c[k+1];if(dir==="LONG"&&z.close<z.open&&n.close>z.high+a*.40)ob=Math.max(ob,30);if(dir==="SHORT"&&z.close>z.open&&n.close<z.low-a*.40)ob=Math.max(ob,30)}if(dir==="LONG"&&x.low<minL-.20*a&&x.close>minL)sweep=40;if(dir==="SHORT"&&x.high>maxH+.20*a&&x.close<maxH)sweep=40;return clamp(fvg+ob+sweep,0,100)}
function supportResistanceScore(c,i,dir){const x=c[i],z=zone(c,i,dir),sw=swings(c,i),a=x.atr||1;let s=0;if(z.dist<=.80)s+=35;if(z.dist<=.30)s+=15;if(z.touches>=2&&z.touches<=6)s+=25;if(z.reactionATR>=.50)s+=15;if(dir==="LONG"&&Math.abs(x.close-sw.lastL)<=a*.80)s+=10;if(dir==="SHORT"&&Math.abs(x.close-sw.lastH)<=a*.80)s+=10;return clamp(s,0,100)}
function supplyDemandScore(c,i,dir){const x=c[i],a=x.atr||1;let score=0;for(let k=Math.max(3,i-60);k<i-1;k++){const z=c[k],n=c[k+1],imp=Math.abs(n.close-z.close)/(a||1);if(dir==="LONG"&&z.close<z.open&&n.close>z.high&&imp>=.50){const dist=Math.abs(x.close-z.low)/a;if(dist<=.80)score=Math.max(score,80-dist*25)}if(dir==="SHORT"&&z.close>z.open&&n.close<z.low&&imp>=.50){const dist=Math.abs(x.close-z.high)/a;if(dist<=.80)score=Math.max(score,80-dist*25)}}return clamp(score,0,100)}
function liquidityScore(c,i,dir){const x=c[i],a=x.atr||1,prior=c.slice(Math.max(0,i-55),i);const lows=prior.map(z=>z.low),highs=prior.map(z=>z.high);const minL=Math.min(...lows),maxH=Math.max(...highs);const eqL=lows.filter(v=>Math.abs(v-minL)<=a*.15).length,eqH=highs.filter(v=>Math.abs(v-maxH)<=a*.15).length;let s=0;if(dir==="LONG"){if(eqL>=2)s+=25;if(x.low<minL-.20*a&&x.close>minL)s+=60;if(x.close>x.open&&x.low<=minL+a*.30)s+=15}else{if(eqH>=2)s+=25;if(x.high>maxH+.20*a&&x.close<maxH)s+=60;if(x.close<x.open&&x.high>=maxH-a*.30)s+=15}return clamp(s,0,100)}
function locationScore(c,i,dir){const x=c[i],a=x.atr||1,z=zone(c,i,dir);const emaDist=Math.min(Math.abs(x.close-x.e21),Math.abs(x.close-x.e55))/(a||1);let s=0;if(z.dist<=.80)s+=45;if(emaDist<=.60)s+=25;if(dir==="LONG"){if(x.close>=x.e55-.30*a)s+=15;if(x.low<=x.e21+.35*a||x.low<=x.e55+.45*a)s+=15}else{if(x.close<=x.e55+.30*a)s+=15;if(x.high>=x.e21-.35*a||x.high>=x.e55-.45*a)s+=15}return clamp(s,0,100)}
...
function momentumScore(c,i,dir){const x=c[i],p3=c[Math.max(0,i-3)]||x;let s=0;if(dir==="LONG"){if(x.rsi>=38&&x.rsi<=60)s+=20;if(x.rsi-x.rsi<=0){} if(x.rsi-p3.rsi>0)s+=15;if(x.rsi<=75)s+=10;if(x.macd>x.macdSig)s+=15;if(x.hist>(c[i-1]?.hist??x.hist)&&(c[i-1]?.hist??x.hist)>=(c[i-2]?.hist??x.hist))s+=15;if(x.plusDI-x.minusDI>=3)s+=10;if(x.stochK>x.stochD&&x.stochK<80)s+=8;if(x.mfi>=45||x.mfi>p3.mfi)s+=7;if(x.cci>-100&&x.cci>(p3.cci||x.cci))s+=5}else{if(x.rsi>=40&&x.rsi<=62)s+=20;if(x.rsi-p3.rsi<0)s+=15;if(x.rsi>=25)s+=10;if(x.macd<x.macdSig)s+=15;if(x.hist<(c[i-1]?.hist??x.hist)&&(c[i-1]?.hist??x.hist)<=(c[i-2]?.hist??x.hist))s+=15;if(x.minusDI-x.plusDI>=3)s+=10;if(x.stochK<x.stochD&&x.stochK>20)s+=8;if(x.mfi<=55||x.mfi<p3.mfi)s+=7;if(x.cci<100&&x.cci<(p3.cci||x.cci))s+=5}return clamp(s,0,100)}
function volumeScore(c,i,dir){const x=c[i];let s=0;if(x.volumeRatio>=1.00)s+=20;if(x.volumeRatio>=1.20)s+=15;if(x.volumeZ>=0)s+=15;if(x.volumeZ>=.50)s+=10;if(x.volumePercentile>=50)s+=10;if(x.volumePercentile>=70)s+=10;if(dir==="LONG"){if(x.cmf>=0)s+=15;if(x.cmf>=.05)s+=5;if(x.obvSlope>0)s+=10}else{if(x.cmf<=0)s+=15;if(x.cmf<=-.05)s+=5;if(x.obvSlope<0)s+=10}return clamp(s,0,100)}
function trendScore(c,i,dir){const x=c[i];let sc=0;if(dir==="LONG"){if(x.e21>=x.e55)sc+=15;if(x.e21>x.e55&&x.e55>x.e100)sc+=15;if(x.ema55SlopeATR>=0)sc+=15;if(x.ema55SlopeATR>=.10)sc+=10;if(x.close>=x.e55-.30*x.atr)sc+=10;if(x.adx>=20)sc+=15;if(x.adx>=25)sc+=5;if(x.plusDI>x.minusDI)sc+=10;if(x.plusDI-x.minusDI>=3)sc+=5}else{if(x.e21<=x.e55)sc+=15;if(x.e21<x.e55&&x.e55<x.e100)sc+=15;if(x.ema55SlopeATR<=0)sc+=15;if(x.ema55SlopeATR<=-.10)sc+=10;if(x.close<=x.e55+.30*x.atr)sc+=10;if(x.adx>=20)sc+=15;if(x.adx>=25)sc+=5;if(x.minusDI>x.plusDI)sc+=10;if(x.minusDI-x.plusDI>=3)sc+=5}return clamp(sc,0,100)}
...
function stopTp(c,i,dir){const x=c[i],sw=swings(c,i),a=x.atr||Math.max(x.high-x.low,1e-9),entry=x.close;let stop,nearTarget;if(dir==="LONG"){stop=Math.min(sw.lastL-.10*a,entry-.80*a);nearTarget=sw.lastH>entry?sw.lastH:entry+a*2.2}else{stop=Math.max(sw.lastH+.10*a,entry+.80*a);nearTarget=sw.lastL<entry?sw.lastL:entry-a*2.2}const risk=Math.abs(entry-stop)||a,stopPct=risk/entry*100,stopAtr=risk/a;const t1=dir==="LONG"?entry+risk*1.05:entry-risk*1.05,t2=dir==="LONG"?entry+risk*1.85:entry-risk*1.85,t3=dir==="LONG"?entry+risk*2.65:entry-risk*2.65;const area=Math.abs(nearTarget-entry)/risk;let stopQ=0;if(stopAtr>=.80&&stopAtr<=2.20)stopQ=100;else if(stopAtr>=.60&&stopAtr<.80)stopQ=60;else if(stopAtr>2.20&&stopAtr<=3.20)stopQ=55;let tpQ=clamp(area/1.80*100,0,100);return{entry,stop,t1,t2,t3,risk,stopPct,stopAtr,tp2Area:area,stopQ:clamp(stopQ,0,100),tpQ}}
function volatilityScore(c,i,st){const x=c[i];let s=0;if(x.atrPct>=.30&&x.atrPct<=6.00)s+=25;if(x.atrPercentile>=20&&x.atrPercentile<=85)s+=25;const rangeATR=(x.high-x.low)/(x.atr||1);if(rangeATR<=2.00)s+=20;if(x.bbWidthPercentile>=15&&x.bbWidthPercentile<=85)s+=15;if(st.stopAtr>=.80&&st.stopAtr<=2.20)s+=15;return clamp(s,0,100)}

function contextGate(scores,st){
  // v16.7 bağlam kapısı: tek tek skor değil, teknik bağların birlikte çalışması ölçülür.
...
function analyzeLast(sym,tf,raw,dir){
  ...
  // v16.7 teknik kapı: önce sadece teknik bağlam ölçülür.
  ...
  // Backtest artık teknik kapıdan sonra sıralama ölçüsüdür; sert ikinci kapı değildir.
  // Ancak backtest hiç ölçülemiyorsa rank 0 kalır ve doğal olarak üst listeye çıkamaz.
  if(bt.count<10)return null;
  ...
  return{sym,tf,dir,model,mode:"TEKNIK KAPI + BACKTEST TOP 5",listMode:"TEKNIK KAPI GEÇTİ / BACKTEST TOP 5",poolClass:poolClass(tech,layerHits),quality:q.cls,main:q.main,rankScore:btRank,btRank,tech,exec,back,dataScore,scores,context:ctx,layerHits,bt,...st,ageSec,source:x.source||"JSON"}
}
...
function backtestRank(bt,st,hits,tech,exec,dataScore){
  // v16.7: Final sıralama SADECE teknik kapıdan geçen adaylar arasında backtest dayanıklılığına göre yapılır.
...
function selectTopByDir(dir){
  ...
  for(const x of arr){
    ...
    x.listMode=`BACKTEST EN İYİ 5 ${dir}`;
    selected.push(x);
    used.add(key);
  }
  return selected;
...
function renderSummary(){
  ...
  $("summary").innerHTML=`<div class="dash"><div><b>${SYMBOLS.length}</b><span>coin evreni</span></div><div><b>${scan.done}/${scan.total}</b><span>sembol/TF analiz</span></div><div><b>${scan.dirChecks}</b><span>yön kontrolü</span></div><div><b>${long.length}</b><span>LONG teknik+BT geçti</span></div><div><b>${short.length}</b><span>SHORT teknik+BT geçti</span></div><div><b>${topLong.length}+${topShort.length}</b><span>ilk 5+5 backtest</span></div><div><b>${scan.rest}</b><span>REST</span></div><div><b>${scan.json}</b><span>JSON</span></div><div><b>${scan.stale}</b><span>canlı alınamadı</span></div><div><b>${scan.out}</b><span>elenen yön</span></div></div><div class="note"><b>v16.7 kuralı:</b> Önce objektif teknik kapı çalışır: trend, üst zaman, piyasa yapısı, Smart Money, destek/direnç, supply-demand, order block/FVG, likidite, lokasyon, momentum, hacim/para akışı, mum tetik, volatilite ve stop/TP alanı sayısal eşiklerle geçilir. Teknik kapıdan geçenlerin backtesti hesaplanır; final yayın backtest dayanıklılığı en iyi 5 LONG ve en iyi 5 SHORT olarak ayrı sıralanır.</div>`
}
function card(x,i){
...
async function oneClickScan(){pool=[];selected=null;scan={done:0,total:0,dirChecks:0,passedTech:0,btPassed:0,out:0,json:0,rest:0,stale:0,invalid:0};$("mainBtn").disabled=true;setMeta("Veri bağlantısı kuruluyor...");setBar(0);await loadMarket();await loadFx();await getUniverse();scan.total=SYMBOLS.length*TFS.length;setDataBox();for(const sym of SYMBOLS){for(const tf of TFS){const raw=await getCandles(sym,tf);scan.done++;setBar(scan.done/scan.total*100);if(raw&&raw.length>=220){for(const dir of ["LONG","SHORT"]){scan.dirChecks++;const c=analyzeLast(sym,tf,raw,dir);if(c)pool.push(c);else scan.out++}}else{scan.dirChecks+=2;scan.out+=2}setMeta(`Tarama ${scan.done}/${scan.total} | ${sym} ${tf} | Teknik kapı+BT geçen ${pool.length} | LONG ${pool.filter(x=>x.dir==='LONG').length} | SHORT ${pool.filter(x=>x.dir==='SHORT').length}`);if(scan.done%8===0){renderList();await new Promise(r=>setTimeout(r,0))}}}pool=dedup(pool).sort((a,b)=>b.rankScore-a.rankScore);renderList();setMeta(`Tarama bitti: ${scan.done}/${scan.total} sembol/TF | Yön kontrolü ${scan.dirChecks} | Teknik kapı+BT geçen ${pool.length} | Backtest en iyi 5 LONG + 5 SHORT hazır`);$("mainBtn").disabled=false}
