const VERSION="v16.13 teknik havuz görünür + aynalı BT denetim";
...
function analyzeLast(sym,tf,raw,dir){
  ...
  // v16.12: Backtest, canlı teknik çekirdeğin aynısını geçmiş mumlarda çalıştırır.
  const bt=backtest(c,dir,sym,tf);
  ...
  const btOk=backtestEligible(bt,back);
  return{sym,tf,dir,model,mode:"AYNALI TEKNIK KAPI + BACKTEST TOP 5",listMode:"TEKNIK HAVUZ / AYNALI BACKTEST",poolClass:poolClass(tech,layerHits),quality:q.cls,main:q.main,btOk,rankScore:btRank,btRank,tech,exec,back,dataScore,scores,context:ctx,layerHits,bt,...st,ageSec,source:x.source||"JSON"}
}
...
function backtest(c,dir,sym="",tf=""){
  ...
  for(let i=160;i<c.length-25;i++){
    ...
    let r=hit==="TP2"?1.85:hit==="TP1"?1.05:hit==="STOP"?-1:(mfe>1?0.4:-0.3);
    trades.push({r,hit,mfe,mae,bars,fast:hit==="STOP"&&bars<=4,tech:ev.tech,exec:ev.exec,layers:ev.layerHits});
    cool=5;
  }
...
function selectTopByDir(dir){
  ...
  for(const x of arr){
    ...
    x.listMode=`TEKNİK HAVUZDAN AYNALI BACKTEST EN İYİ 5 ${dir}`;
    selected.push(x);
    used.add(key);
  }
  return selected;
...
function btFailReasons(x){
  const bt=x.bt||{};
  const reasons=[];
  const mfeMae=bt.avgMae>0?bt.avgMfe/bt.avgMae:(bt.avgMfe||0);
  if((bt.count||0)<BACKTEST_GATE.minCount)reasons.push(`işlem ${bt.count||0}<${BACKTEST_GATE.minCount}`);
  if((bt.win||0)<BACKTEST_GATE.minWin)reasons.push(`win ${fmt(bt.win||0,1)}<${BACKTEST_GATE.minWin}`);
  if((bt.pf||0)<BACKTEST_GATE.minPf)reasons.push(`PF ${fmt(bt.pf||0,2)}<${BACKTEST_GATE.minPf}`);
  if((bt.fast||0)>BACKTEST_GATE.maxFast)reasons.push(`hızlı stop ${fmt(bt.fast||0,1)}>${BACKTEST_GATE.maxFast}`);
  if((bt.net||0)<=BACKTEST_GATE.minNet)reasons.push(`netR ${fmt(bt.net||0,2)}≤${BACKTEST_GATE.minNet}`);
  if(mfeMae<BACKTEST_GATE.minMfeMae)reasons.push(`MFE/MAE ${fmt(mfeMae,2)}<${BACKTEST_GATE.minMfeMae}`);
  if((x.back||0)<BACKTEST_GATE.minHealth)reasons.push(`BT sağlık ${Math.round(x.back||0)}<${BACKTEST_GATE.minHealth}`);
  if(x.quality==='BT-C')reasons.push('sınıf BT-C');
  return reasons.length?reasons.join(' | '):'uygun';
}

function renderSummary(){
  const long=pool.filter(x=>x.dir==="LONG"),short=pool.filter(x=>x.dir==="SHORT");
  const eligLong=long.filter(x=>x.btOk),eligShort=short.filter(x=>x.btOk);
...
function renderList(){
  ...
  const longHtml=topLong.length?topLong.map((x,i)=>card(x,i)).join(''):'<p>LONG tarafında aynalı backtest uygunluk kilidini geçen final aday yok.</p>';
  const shortHtml=topShort.length?topShort.map((x,i)=>card(x,i)).join(''):'<p>SHORT tarafında aynalı backtest uygunluk kilidini geçen final aday yok.</p>';
