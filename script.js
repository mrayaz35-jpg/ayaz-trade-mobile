...
const RULE={capital:100,riskPct:2.5,maxLev:6,minConf:62,liveOnlyMinConf:58,maxLiveAgeMs:300000,maxStopPct:3.8,minRR:1.20,minTrades:6,minPF:1.05,minWin:45,minMfeMae:0.75,cooldown:6,minConfluence:4,minZoneTouch:1,forceTopCount:10,maxConf:94,maxDisplay:10,liveRequired:true,allowHighRiskTop:false};
let market={generatedAt:null,data:{}},wsList=[],wsOk=false,wsLast=0,liveMap={},sourceMap={},candidates=[],selected=null,scanLog={total:0,done:0,rest:0,ws:0,json:0,skipped:0},fx={rate:null,source:'-',updatedAt:null,ageSec:null},dataHealth={json:false,rest:false,ws:false,lastError:'-',candles:0};
...
function pairSourceTime(sym,tf,arr){
  ...
  // v14.4 CANLI ZORUNLU:
  // İşlem kararı için mumun açılış/kapanış saati değil, verinin gerçekten çekildiği zaman kullanılır.
  // Böylece 1h/2h/4h mumlarında eski JSON, mum kapanışı gelecekte diye taze sanılmaz.
  return Math.max(Number(last.liveTime)||0,safeTs(market&&market.generatedAt)||0);
}
...
function getCandles(s,tf){return market?.data?.[s]?.[tf]||[]}async function ensureCandles(s,tf){
  ...
  if(arr.length>=220 && pairLive && ageMs<=RULE.maxLiveAgeMs){
    liveMap[key]=pairLive;sourceMap[key]='JSON TAZE';
...
function adjustRiskConfidence(obj){
  if(!obj)return obj;
  if(obj.riskClass==='Yüksek'){obj.rawConf=Math.min(obj.rawConf,78);obj.conf=Math.min(obj.conf,78);obj.capNotes=[...(obj.capNotes||[]),'yüksek risk güven tavanı'];}
...
function buildCandidate(sym,tf,b,stat){
  ...
  return adjustRiskConfidence({sym,tf,model:sig.model,sub:sig.sub,dir:sig.dir,conf,rawConf,cap:capInfo.cap,capNotes:["canlı/taze veri", ...capInfo.notes],riskClass:rClass,entry:sig.entry,stop:sig.stop,t1:sig.t1,t2:sig.t2,t3:sig.t3,stopPct:sig.stopPct,rr:sig.rr,why:sig.why,stat,lev,candles:c,ageSec:Math.max(0,Math.round(ageMs/1000)),source:sourceMap[sym+'|'+tf]||'Taze veri',liveOnly:false,stale:false})
}
...
function buildLiveCandidate(sym,tf,raw,b){
  ...
  for(const dir of ["LONG","SHORT"]){
    ...
    out.push(adjustRiskConfidence({sym,tf,model:sig.model||("Canlı Bağlam "+dir),sub:(sig.sub||"canlı bağlam"),dir:sig.dir,conf,rawConf,cap:hasStat?RULE.maxConf:76,capNotes:hasStat?["canlı/taze veri", "canlı fırsat filtresi"]:["canlı/taze veri", "backtest örneklemi yetersiz", "canlı bağlam öncelikli"],riskClass:rClass,entry:last.close,stop:sig.stop,t1:sig.t1,t2:sig.t2,t3:sig.t3,stopPct:sig.stopPct,rr:sig.rr,why:sig.why,stat,lev,candles:c,ageSec:Math.max(0,Math.round(ageMs/1000)),source:sourceMap[sym+'|'+tf]||'Taze veri',liveOnly:!hasStat,stale:false}));
  }
...
function buildContextFallbackCandidate(sym,tf,raw,b){
  ...
  function make(dir){
    ...
    return adjustRiskConfidence({sym,tf,model:'Canlı Yedek Bağlam '+dir,sub:sub.slice(0,2).join(' + ')||'en güçlü 10 doldurma',dir,conf,rawConf:Math.round(score),cap:82,capNotes:['canlı/taze veri','sert sinyal yoksa bağlam adayı'],riskClass:rClass,entry,stop,t1,t2,t3,stopPct,rr,why:[...new Set(why)].slice(0,6),stat,lev,candles:c,ageSec:Math.max(0,Math.round(ageMs/1000)),source:sourceMap[sym+'|'+tf]||'Taze veri',liveOnly:true,stale:false,contextFallback:true});
  }
  const arr=[make('LONG'),make('SHORT')].filter(Boolean).sort((a,b)=>b.conf-a.conf);
  return arr[0]||null;
}
function sortCandidates(){
  candidates.sort((a,b)=>{const ra=a.riskClass==='Yüksek'?1:0,rb=b.riskClass==='Yüksek'?1:0;if(ra!==rb)return ra-rb;return b.conf-a.conf||a.ageSec-b.ageSec});
...
async function scanAll(){
  ...
  setMeta(`${liveText()} | Tarama bitti: ${done}/${total} kontrol | Coin evreni: ${SYMBOLS.length} | JSON: ${scanLog.json} | REST: ${scanLog.rest} | WS: ${scanLog.ws} | Gösterilen: ${Math.min(RULE.maxDisplay,candidates.length)} | Taze veri sınırı: 5 dk | Eski JSON: işlem kapalı | v14.5 risk filtresi aktif`);
}
function renderList(){
  const box=$("list");
  if(!candidates.length){
    box.innerHTML='<div class="decision wait">ADAY HENÜZ YOK</div><p>Tarama devam ediyor veya canlı/taze veri bekleniyor. v14.4 eski JSON’dan işlem açmaz; REST/WS ya da 5 dakikadan taze market.json gerekir.</p>';
    return;
  }
  box.innerHTML=candidates.slice(0,RULE.maxDisplay).map((x,i)=>{
    ...
    return `<div class="candidate ${x.dir==='SHORT'?'short':'long'}" onclick="selectCandidate(${i})"><div class="top"><div><div class="sym">${i+1}) ${cleanSymbol(x.sym)||x.sym} / ${x.tf}</div><div class="model">${x.model}${x.sub?" — "+x.sub:""}</div></div><div class="score ${x.dir==='LONG'?'long':'short'}">${x.conf}%<br><span style="font-size:16px">${x.dir}</span></div></div><div class="line">Giriş ${dualPrice(x.entry)}<br>Stop ${dualPrice(x.stop)}<br>TP1 ${dualPrice(x.t1)} | TP2 ${dualPrice(x.t2)} | TP3 ${dualPrice(x.t3)}<br>Kaldıraç x${fmt(x.lev.lev,1)} | Pozisyon ${dualMoney(x.lev.notional)} | Marjin ${dualMoney(x.lev.margin)} | Risk ${dualMoney(x.lev.riskD)} | Stop ${pct(x.stopPct,2)}<br>Backtest: ${bt} | Veri ${x.ageSec} sn | Kaynak: ${x.source||'Taze veri'}<br>Risk sınıfı: ${x.riskClass} | Ham güven ${x.rawConf}% → Gerçekçi güven ${x.conf}%${x.liveOnly?' | CANLI ÖNCELİKLİ':''}</div></div>`;
  }).join("")
}
function selectCandidate(i,auto=false){
  ...
  $("metrics").innerHTML=metric("Sembol / TF",`${cleanSymbol(x.sym)||x.sym} / ${x.tf}`)+metric("Model",x.model+(x.sub?" — "+x.sub:""))+metric("Canlı veri",`${x.ageSec} sn`)+metric("Veri kaynağı",x.source||"Taze veri")+metric("Risk sınıfı",x.riskClass)+metric("Ham/Gerçekçi güven",`${x.rawConf}% / ${x.conf}%`)+metric("Giriş",dualPrice(x.entry))+metric("Stop",dualPrice(x.stop))+metric("Stop %",pct(x.stopPct,2))+metric("TP1",dualPrice(x.t1))+metric("TP2",dualPrice(x.t2))+metric("TP3",dualPrice(x.t3))+metric("Kaldıraç",`x${fmt(x.lev.lev,1)}`)+metric("Pozisyon",dualMoney(x.lev.notional))+metric("Marjin",dualMoney(x.lev.margin))+metric("Risk",dualMoney(x.lev.riskD))+metric("RR",fmt(x.rr,2))+metric("PF",x.liveOnly?"Canlı bağlam":(x.stat.pf>=20?"20+":fmt(x.stat.pf,2)));
...
function binanceTryPlan(x){
  ...
  if(x.dir==="LONG"){
    ...
    return `${warn}<b>Binance TR LONG emir özeti</b><div class="tryline">Limit AL fiyatı: ${tlInput(x.entry)} TL</div><div class="tryline">OCO SAT: Kar al ${tlInput(x.t1)} TL | Stop ${tlInput(x.stop)} TL | Stop-limit ${tlInput(stopLimit)} TL</div><div class="tryline">Pozisyon: ${tlMoney(x.lev.notional)} | Marjin: ${tlMoney(x.lev.margin)} | Risk: ${tlMoney(x.lev.riskD)}</div><div class="tryline">TP2: ${tlInput(x.t2)} TL | TP3: ${tlInput(x.t3)} TL</div>`;
  }
  return `${warn}<b>SHORT planı</b><div class="tryline">Bu yön Binance TR spotta doğrudan short olarak açılamaz. Vadeli/marjin altyapısı gerekir; şimdilik izleme sinyali gibi değerlendir.</div><div class="tryline">Giriş: ${tlInput(x.entry)} TL | Stop: ${tlInput(x.stop)} TL | TP1: ${tlInput(x.t1)} TL</div>`;
