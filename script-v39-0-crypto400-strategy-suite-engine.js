window.__AYAZ_ACTIVE_VERSION__="v39.0-strategy-suite-engine";
const VERSION="v39.0 KRİPTO STRATEJİ MODÜLLERİ + PROFESYONEL BAĞLAM MOTORU";
...
const RULE={
  ...
  minBacktestTrades:3, minBacktestWin:32, minBacktestPf:0.95, minBacktestNetR:-0.50,
  maxFastStopRate:70, minTp2Rate:0, minMfeMae:0.60,
  adaptiveMinTrades:1, adaptivePf:0.85, adaptiveNetR:-0.25, adaptiveFastStop:75, adaptiveTp2Rate:0, adaptiveMfeMae:0.50,
  minLiveVolRatio:0.30, atrBuffer:0.25, minTp1R:0.90, minTp2R:1.25
};
function maxStopAllowed(tf){return ({"15m":6.20,"30m":6.80,"1h":8.00,"2h":9.00,"4h":10.50}[tf]||8.00)}
function minBaseStopAllowed(tf){return ({"15m":0.35,"30m":0.40,"1h":0.45,"2h":0.50,"4h":0.55}[tf]||0.45)}
function minFastStopAllowed(tf){return ({"15m":0.55,"30m":0.60,"1h":0.70,"2h":0.80,"4h":0.90}[tf]||0.65)}
function minAtrStopAllowed(tf,ev){return structuralConfirmation(ev)?({"15m":0.55,"30m":0.60,"1h":0.70,"2h":0.80,"4h":0.90}[tf]||0.65):({"15m":0.85,"30m":0.90,"1h":1.00,"2h":1.10,"4h":1.20}[tf]||0.95)}
...
function contextTag(bt,p,ev,ageSec){
  ...
  const retestOkForProfile=!(ev.profile?.type==='KIRILIM_RETEST') || ev.gates.retest;
  const minStop=minStopAllowed(ev.tf||'1h',ev);
  const fastStopGuardOk=p.stopPct>=minStop;
  const targetRoomOk=p.tp1ReachR>=RULE.minTp1R && p.tp2ReachR>=RULE.minTp2R;
  const nakedTrend=ev.profile?.type==='TREND_DEVAM'&&!structuralConfirmation(ev);
  const room=directionalRoom(ev,ev.dir||'LONG');
  const structureOk=hasMajorStructure(ev);
  const triggerQuality=ev.candle.ok||ev.liquidity.ok||ev.gates.strongBreak||ev.gates.retest||ev.gates.emaRetest||ev.gates.strongBody;
  const volumeSoftOk=ev.candle.volRatio>=0.65 || ev.gates.volumeOk || ev.gates.strongBody || structureOk;
  const nakedTrendOk=!nakedTrend || (p.tp1ReachR>=1.00 && p.tp2ReachR>=1.35 && (ev.liquidity.ok||ev.gates.emaRetest||ev.gates.nearLevel||ev.gates.srBreak));
  const riskOk=fastStopGuardOk && targetRoomOk && nakedTrendOk && p.stopPct<=maxStop && !p.riskCapped && p.structuralRiskPct<=maxStop && stopSideOk && retestOkForProfile;
  const livePowerOk=ev.mtf.ok && ev.gates?.groupsOk && ev.gates.groupPass>=4 && ev.ind.passed>=4 && ev.gates.enoughRoom && ev.gates.triggerOk && ev.gates.locationOk && triggerQuality && volumeSoftOk;
  ...
  const adaptiveBtOk=bt.count>=RULE.adaptiveMinTrades && bt.pf>=RULE.adaptivePf && bt.net>=RULE.adaptiveNetR && bt.fast<=RULE.adaptiveFastStop && bt.tp2Rate>=RULE.adaptiveTp2Rate && mfeMae>=RULE.adaptiveMfeMae && ev.gates.groupPass>=4 && riskOk && livePowerOk;
  const btOk=classicBtOk||adaptiveBtOk;
  const strategyTrendOk=ev.mtf.ok && (ev.gates.trendAligned||structureOk);
  const strategyStructureOk=structureOk || ev.structure.choch || ev.structure.bos;
  const strategySRLineOk=ev.gates.locationOk && (ev.gates.nearLevel||ev.gates.srBreak||ev.gates.lineBreak||ev.gates.retest||ev.gates.emaRetest);
  const strategyIndicatorOk=ev.ind.passed>=5 || (ev.ind.passed>=4&&ev.gates.momentumOk);
  const strategyCandleOk=triggerQuality && (volumeSoftOk||ev.gates.strongBody||ev.gates.retest);
  const strategyLiquidityOk=ev.liquidity.ok || ev.gates.retest || ev.gates.srBreak || ev.structure.bos;
  const strategyRiskOk=riskOk && targetRoomOk;
  const strategyBtOk=btOk || (bt.count>=1 && bt.pf>=1.00 && bt.fast<=80);
  const strategyPassCount=[strategyTrendOk,strategyStructureOk,strategySRLineOk,strategyIndicatorOk,strategyCandleOk,strategyLiquidityOk,strategyRiskOk,strategyBtOk].filter(Boolean).length;
  const strategyLines=[
    `1) Trend/MTF Stratejisi: ${strategyTrendOk?'GEÇTİ':'ZAYIF'} | Ana TF ${ev.trend.state}, üst TF ${ev.mtf.label}`,
    `2) Yapı Karakter Stratejisi: ${strategyStructureOk?'GEÇTİ':'ZAYIF'} | BOS ${yn(ev.structure.bos)}, CHOCH ${yn(ev.structure.choch)}, SR ${yn(ev.gates.srBreak||ev.gates.lineBreak)}, Retest ${yn(ev.gates.retest)}`,
    `3) Destek-Direnç/Çizgi/Dip-Tepe: ${strategySRLineOk?'GEÇTİ':'ZAYIF'} | ${ev.setupZone}`,
    `4) İndikatör/Ortalama Stratejisi: ${strategyIndicatorOk?'GEÇTİ':'ZAYIF'} | EMA/RSI/MACD/ADX-DI ${ev.ind.passed}/${ev.ind.total}`,
    `5) Mum+Hacim Stratejisi: ${strategyCandleOk?'GEÇTİ':'ZAYIF'} | ${ev.candle.name}, hacim x${fmt(ev.candle.volRatio,2)}`,
    `6) Likidite/SMC Stratejisi: ${strategyLiquidityOk?'GEÇTİ':'ZAYIF'} | Sweep/Reclaim ${yn(ev.liquidity.ok)}, OB/FVG alanı lokasyonla birlikte değerlendirilir`,
    `7) ATR Stop/Hedef Stratejisi: ${strategyRiskOk?'GEÇTİ':'ZAYIF'} | Stop ${pct(p.stopPct,2)}, TP1 ${fmt(p.tp1ReachR,2)}R, TP2 ${fmt(p.tp2ReachR,2)}R`,
    `8) Aynı-Strateji Backtest: ${strategyBtOk?'GEÇTİ':'ZAYIF'} | ${ev.primary}, işlem ${bt.count}, PF ${bt.pf>=20?'20+':fmt(bt.pf,2)}, fast ${pct(bt.fast,1)}`
  ];
  const professionalOk=dataOk && riskOk && livePowerOk && strategyPassCount>=4 && (structureOk || room>=1.40 || ev.gates.retest || ev.gates.emaRetest);
  const eliteOk=professionalOk && ev.mtf.ok && strategyPassCount>=6 && ev.gates.groupPass>=5 && ev.ind.passed>=5 && p.tp2ReachR>=1.35 && (btOk||bt.count>=1);
  const tradeOk=dataOk && riskOk && livePowerOk && strategyPassCount>=4 && (btOk || eliteOk || structureOk || ev.gates.retest || ev.gates.emaRetest || room>=1.70);
  const state=tradeOk?'TRADE EDİLEBİLİR':'ELENDİ';
  const tier=eliteOk?'ELİT':professionalOk?'PROFESYONEL ADAY':'ELENDİ';
  const tierRank=eliteOk?3:professionalOk?2:0;
  const passList=[];
  const failList=[];
  function add(name,ok,detail){(ok?passList:failList).push(detail?`${name}: ${detail}`:name)}
  add('Trend/Üst TF uyumu',ev.mtf.ok,ev.mtf.label);
  add('Lokasyon',ev.gates.locationOk,ev.setupZone);
  add('Yapı teyidi',structureOk,`BOS ${ev.structure.bos?'EVET':'HAYIR'}, CHOCH ${ev.structure.choch?'EVET':'HAYIR'}, SR ${ev.gates.srBreak||ev.gates.lineBreak?'EVET':'HAYIR'}, Retest ${ev.gates.retest?'EVET':'HAYIR'}`);
  add('Tetik kalitesi',triggerQuality,ev.candle.name);
  add('Hacim/Gövde',volumeSoftOk,`Hacim x${fmt(ev.candle.volRatio,2)}`);
  add('Momentum',ev.gates.momentumOk,`İndikatör ${ev.ind.passed}/${ev.ind.total}`);
  add('Hedef alanı',targetRoomOk,`TP1 ${fmt(p.tp1ReachR,2)}R / TP2 ${fmt(p.tp2ReachR,2)}R`);
  add('Stop/ATR güvenliği',riskOk,`Stop ${pct(p.stopPct,2)}, min ${pct(minStop,2)}, max ${pct(maxStop,2)}`);
  add('Backtest',btOk,`${classicBtOk?'KLASİK':adaptiveBtOk?'ADAPTİF':'ZAYIF'} | İşlem ${bt.count}, PF ${bt.pf>=20?'20+':fmt(bt.pf,2)}, Fast ${pct(bt.fast,1)}`);
  return {state,tradeOk,btOk,classicBtOk,adaptiveBtOk,livePowerOk,mfeMae,riskOk,dataOk,maxStop,minStop,fastStopGuardOk,targetRoomOk,nakedTrend,nakedTrendOk,primary:ev.primary,strategies:ev.strategies,professionalOk,eliteOk,tier,tierRank,passList,failList,structureOk,volumeSoftOk,triggerQuality,strategyLines,strategyPassCount};
}
