window.__AYAZ_ACTIVE_VERSION__="v21.0-crypto-only-400";
const VERSION="v21.0 SADECE KRİPTO 400 S9 DİNAMİK";
...
function bistProxyUrls(url){const e=encodeURIComponent(url);const url2=url.replace('query1.finance.yahoo.com','query2.finance.yahoo.com');const e2=encodeURIComponent(url2);return [
  {url,mode:'raw'},
  {url:url2,mode:'raw'},
  ...
  {url:`https://thingproxy.freeboard.io/fetch/${url}`,mode:'raw'},
  {url:`https://api.codetabs.com/v1/proxy/?quest=${e}`,mode:'raw'},
  {url:`https://api.codetabs.com/v1/proxy?quest=${e2}`,mode:'raw'}
]}
...
async function getBistSets(code){
  const sym=code+'.IS';
  // 1) Önce GitHub Actions'ın ürettiği aynı-origin BIST JSON köprüsünü kullan.
  // GitHub Pages'te CORS sorunu yaşamayan en sağlam BIST katmanı budur.
  await loadBistBridge(false);
  const js=bridgeSets(code);
  if(Object.values(js).some(a=>a&&a.length>=80))return js;
  // 2) JSON köprüsü henüz oluşmadıysa tarayıcıdan Yahoo/proxy canlı-yakın veri dene.
...
function dataLine(){return `KRİPTO | Geçerli set ${stats.sets}/${stats.done} | Mum ${stats.candles} | Kaynak ${stats.source} | Yaş ${stats.age} sn | ${VERSION}<br>${fxLine()}`}
function summary(){
  const cL=pools.crypto.LONG.length,cS=pools.crypto.SHORT.length;
  $('summary').innerHTML=`<div class="dash"><div><b>${cryptoSymbols.length}</b><span>kripto evreni</span></div><div><b>${stats.done}/${stats.total}</b><span>kripto tarama</span></div><div><b>${stats.sets}</b><span>geçerli set</span></div><div><b>${stats.candles}</b><span>mum</span></div><div><b>${cL}</b><span>LONG aday</span></div><div><b>${cS}</b><span>SHORT aday</span></div></div><div class="note"><b>v21.0:</b> BIST tamamen devre dışıdır. Sistem yalnızca Binance canlı kripto verisiyle çalışır. Evren Binance 24s hacimden dinamik en fazla 400 USDT paritesi seçer. S9 stratejisi: trend kırılım + CHOCH. Dinamik nöbet her döngüde en iyi 5 LONG ve en iyi 5 SHORT listesini revize eder. USDT fiyatları canlı USDT/TRY ile TL’ye çevrilir.</div>`
}
...
function render(){
  ...
  all.forEach((x,i)=>x.key=x.key||`${x.market}_${x.sym}_${x.tf}_${x.dir}_${i}`);
  window.__map=new Map(all.map(x=>[x.key,x]));
  function sec(dir,label){
    const arr=topBy('crypto',dir);
    return `<div class="section ${dir==='LONG'?'long':'short'}"><h3>${label}</h3>${arr.length?arr.map((x,i)=>card(x,i)).join(''):'<p>Aday yok.</p>'}</div>`
  }
  $('list').innerHTML=sec('LONG','KRİPTO Trade Edilebilir İlk 5 LONG')+sec('SHORT','KRİPTO Trade Edilebilir İlk 5 SHORT');
  const first=topBy('crypto','LONG')[0]||topBy('crypto','SHORT')[0];
  if(first&&!selected)selectCandidate(first.key,true);
  $('dataBox').innerHTML=dataLine()
}
...
async function oneClickAllScan(){watchActive=false;activeWatchMarket=null;if(watchTimer)clearTimeout(watchTimer);resetPools('crypto');await runMarketScan('crypto',false)}
