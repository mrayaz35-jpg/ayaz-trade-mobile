window.__AYAZ_ACTIVE_VERSION__="v36.0-symmetric-scientific-trade-engine";
const VERSION="v36.0 KRİPTO SİMETRİK SCIENTIFIC ATR STOP + TARGET MOTORU";
...
const RULE={
  ...
  minBacktestTrades:5, minBacktestWin:36, minBacktestPf:1.05, minBacktestNetR:0.20,
  maxFastStopRate:45, minTp2Rate:8, minMfeMae:0.85,
  adaptiveMinTrades:3, adaptivePf:1.35, adaptiveNetR:1.00, adaptiveFastStop:42, adaptiveTp2Rate:10, adaptiveMfeMae:0.85,
...
function minBtTrades(tf){return ({"15m":5,"30m":5,"1h":6,"2h":6,"4h":7}[tf]||5)}
...
function backtest(c,dir,strategyName,sets=null,tf='1h'){
  ...
  for(let i=150;i<c.length-45;i++){
    ...
    if(p.riskCapped||p.stopPct<minStopAllowed(tf,ev)||p.structuralRiskPct>maxStopAllowed(tf)||p.tp1ReachR<RULE.minTp1R||p.tp2ReachR<RULE.minTp2R)continue;
